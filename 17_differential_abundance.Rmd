...
as a whole. Be aware that this approach has also been critisized
...
[Nearing et al.](https://www.biorxiv.org/content/10.1101/2021.05.10.443486v1.full) who compared all these listed methods across 38
...
methods. Unfortunately, as Nearing et al. point out, they differ disturbingly
...
on the method. In this section we demonstrate 3 methods 
(ANCOM-BC, Wilcoxon-test and DESeq2) and will compare the results between them.
...
ignore for simplicity. Furthermore, we picked a dataset that merely has
...
```{r load-pkg-data}
# packages we use in this tutorial and whether they will be installed via 
# Bioconductor.
required_pkg <- list(
  "patchwork" = "CRAN",
  "devtools" = "CRAN",
...
for (pkg in names(required_pkg)) {
  if (required_pkg[[pkg]] == "BIOC" & !pkg %in% installed.packages()) {
    BiocManager::install(pkg)
  } else if (required_pkg[[pkg]] == "CRAN" & !pkg %in% installed.packages()) {
...
non-parametric alternative to a t-test. As multiple non-independent tests will
be performed, we will need to adjust the p-values for multiple testing. Below
code shows how to do both steps:
...
# select all genera that we perform the test on
genera <- select(daa, -Sample_ID, -x) %>% colnames()
# the map function is very similar to a "for loop". Here, for each genus in
# the vector "genera", perform the actions in {} while genus is a place holder 
# for each genus. Because we chose map_dfr, results will be binded rowwise into 
# a dataframe.
...
### DESeq2
Our second analysis method is DESeq2. DESeq2 utilizes the negative binomial
distribution to detect differences in read counts between groups. It performs a
data normalization automatically that accounts for differences between library
sizes. It will also automatically output adjusted p-values. More information
about this method can be found, e.g., from Harvard Chan Bioinformatic Core's
tutorial [Introduction to DGE -
ARCHIVED](https://hbctraining.github.io/DGE_workshop/lessons/04_DGE_DESeq2_analysis.html)
 
Here is the R code corresponding to the DESeq2 analysis:
...
ggplot(df, aes(padj)) +
  geom_histogram()
# order the rows of the results dataframe in decreasing order based on the
# column "log2FoldChange" and secondly based on the column "padj" in ascending
# order, then show the top 6 only.
arrange(df, desc(log2FoldChange), padj) %>%
  head() %>%
  kable()
```
...
(e.g. adjusted p value < 0.05).  That is what we illustrate here. First let us
print how many taxa were identified by each method to begin:
...
In the next step we will look at the intersection of identified taxa. Here we
encountered a common issue that we want to warn you about regarding the names
of taxa.  R might change these names, e.g. if you generate a dataframe while
the given names are not suitable to be column names. This happened here with
`Escherichia/Shigella`, which was changed to `Escherichia.Shigella`
automatically by R in the Wilcoxon test section. That caused some of the `NA`
values where we expected p values (DESeq2 and Wilcoxon output) or boolean
values (ANCOMBC output). Make sure to always inspect why you do not get the
output you would have expected. Since `Escherichia/Shigella` was FALSE
(adjusted p > 0.05) for all methods, I could easily work around this below by
setting the `NA` to FALSE.
...
summ <- full_join(
  ...
  # rename according to which padj value belong to which method
  select(genus, wilcoxon = p_adjusted, deseq2 = padj, ancombc = phenoObese) %>%
  # apply decision criterion of padj <= 0.05; address issue based on naming 
...
We see that while each method at least identified 14 genera as differentially
abundant, they only agree on 5 genera. Lets plot those 5 genera:
...
Other methods such as both ANCOM methods, DESeq2, MaAsLin2 and others allow
this. Below we show how to include a covariate in ANCOM-BC. See e.g.
[here](https://hbctraining.github.io/DGE_workshop/lessons/04_DGE_DESeq2_analysis.html) on how to do the same for DESeq2. 
