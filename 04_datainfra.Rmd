...
For the sake of demonstration, here we will extract a subset containing only the samples of human origin (feces, skin or tongue), stored as `SampleType` within `colData(se)`.
...
unique(colData(se)$SampleType)
```
```{r eval = FALSE}
# show recurrence for each value
colData(se)$SampleType %>% table()
```
```{r echo = FALSE}
# show recurrence for each value
colData(se)$SampleType %>% table() %>% kable() %>%
...
```{r}
# subset by sample
se_subset_by_sample <- se[ , colData(se)$SampleType == "Feces" | colData(se)$SampleType == "Skin" | colData(se)$SampleType == "Tongue"]
# show dimensions
...
Because the expression within the brackets has to be logical (whether TRUE or FALSE), the terms separated by the OR operator `|` are enclosed within parentheses. Then, the term after the AND operator `&` filters out all the features that present a NA in place of Phylum.

```{r}
# subset by feature
se_subset_by_feature <- se[(rowData(se)$Phylum == "Actinobacteria" | rowData(se)$Phylum == "Chlamydiae") & !is.na(rowData(se)$Phylum), ]
# show dimensions
...
se_phylum_subset_by_feature <- se_phylum[(rowData(se_phylum)$Phylum == "Actinobacteria" | rowData(se_phylum)$Phylum == "Chlamydiae") & !is.na(rowData(se_phylum)$Phylum), ]
# show dimensions
...
se_subset_by_sample_feature <- se[(rowData(se)$Phylum == "Actinobacteria" | rowData(se)$Phylum == "Chlamydiae") & !is.na(rowData(se)$Phylum), colData(se)$SampleType == "Feces" | colData(se)$SampleType == "Skin" | colData(se)$SampleType == "Tongue"]
# show dimensions
...
If a study was to consider and quantify the presence of Actinobacteria as well as Chlamydiae in different sites of the human body, `se_subset_by_sample_feature` might be an ideal subset to start with.
