...
let
  pkgs = import (fetchTarball
    "https://github.com/rstats-on-nix/nixpkgs/archive/bc30fa6c937a4598564bd0fa7114cbad2a3554aa.tar.gz")
    { };
  # Add generic R packages required for this build
  rpkgs = with pkgs.rPackages; [ BiocManager BiocBook ];
    ...
    (pkgs.rPackages.buildRPackage {
      ...
      propagatedBuildInputs = builtins.attrValues {
        inherit (pkgs.rPackages)
          ...
          MultiAssayExperiment MASS ape decontam vegan BiocGenerics S4Vectors
          IRanges Biostrings DECIPHER BiocParallel DelayedArray
          DelayedMatrixStats scuttle scater DirichletMultinomial rlang dplyr
          tibble tidyr bluster rgl MatrixGenerics;
      };
    })
  ];
  ...
  miatime = [
    (pkgs.rPackages.buildRPackage {
      ...
      propagatedBuildInputs = builtins.attrValues {
        inherit (pkgs.rPackages)
          dplyr S4Vectors SummarizedExperiment SingleCellExperiment vegan;
      } ++ [ mia ];
    })
  ];
  ...
  spieceasi = [
    (pkgs.rPackages.buildRPackage {
      ...
      propagatedBuildInputs = builtins.attrValues {
        inherit (pkgs.rPackages)
          huge pulsar MASS VGAM Matrix glmnet RcppArmadillo;
      };
    })
  ];
  ...
  spring = [
    (pkgs.rPackages.buildRPackage {
      ...
      propagatedBuildInputs = builtins.attrValues {
        inherit (pkgs.rPackages) mixedCCA huge pulsar rootSolve mvtnorm;
      } ++ [ spieceasi ];
    })
  ];
  ...
  netcomi = [
    (pkgs.rPackages.buildRPackage {
      ...
      propagatedBuildInputs = builtins.attrValues {
        inherit (pkgs.rPackages)
          Biobase corrplot doSNOW fdrtool filematrix foreach gtools huge igraph
          MASS Matrix mixedCCA orca phyloseq pulsar qgraph RColorBrewer Rdpack
          rlang vegan WGCNA;
      } ++ [ spring ];
    })
  ];
  ...
  oma = [
    (pkgs.rPackages.buildRPackage {
      ...
      propagatedBuildInputs = builtins.attrValues {
        inherit (pkgs.rPackages)
          rebook glue sessioninfo microbiomeDataSets curatedMetagenomicData
          microbiome ggsignif SummarizedExperiment TreeSummarizedExperiment
          kableExtra dendextend NbClust randomcoloR cobiclust biclust tidyverse
          ALDEx2 ANCOMBC Maaslin2 MicrobiomeStat GUniFrac devtools
          ComplexHeatmap mikropml MLeval sechm ggpubr fido rgl miaViz
          SuperLearner multiview MMUPHin gsEasy topGO;
      } ++ [ miatime spieceasi ];
    })
  ];
  ...
  # System dependencies
  system_packages =
    builtins.attrValues { inherit (pkgs) R glibcLocales quarto; };
...
in pkgs.mkShell {
  ...
  buildInputs = [ R rstudio_pkgs system_packages ];
}
