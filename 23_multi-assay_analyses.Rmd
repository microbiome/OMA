...
Multi-omics means data integration from multiple sources. For
example, we can integrate microbial abundances in the gut with
biomolecular profiling data from blood samples. This kind of
integrative multi-omic approach can support the analysis of
microbiome dysbiosis and facilitate the discovery of novel biomarkers
for health and disease.

With cross-correlation analysis, we can analyze how strongly and
differently, variables are associated with each other. For instance,
we can investigate whether a higher presence of a specific taxon 
corresponds to higher levels of a biomolecule.

The data containers that the _miaverse_ utilizes are scalable, and they
can contain different types of data in a same container. As a result,
the _miaverse_ is well-suited to multi-assay microbiome data
...
Multiple experiments are initially imported into the SE or TreeSE data 
containers, similar to when a single experiment is present. After that 
different experiments are combined into the same data container. 
The result is one TreeSE object with alternative experiment in altExp slot,
or MAE object with multiple experiment in its experiment slot.
...
different biomarkers. [Here](https://microbiome.github.io/OMA/containers.html#loading-experimental-microbiome-data) 
...
Next we can do the cross-correlation analysis.  Here, we analyse whether
individual bacteria genera correlate with concentrations of
individual metabolites. This helps as to answer the question: "If this
bacterium is present, is the concentration of this metabolite low or
high"?
