...
taxa_dendrogram <- as.dendrogram(taxa_hclust)
...
# Order data so that it's same as in dendogram
taxa_clusters <- taxa_clusters[order(match(rownames(taxa_clusters), 
                                           labels(taxa_dendrogram))), , drop = FALSE] 
...
# Creates clusters
sample_clusters <- cutree(tree = sample_hclust, k = 3)

# Converts into data frame
sample_clusters <- data.frame(clusters = sample_clusters)

# Order data so that it's same as in dendogram
sample_clusters <- sample_clusters[order(match(rownames(sample_clusters), 
                                               labels(sample_dendrogram))), , drop = FALSE] 

# Order data based on 
tse_phylum_subset <- tse_phylum_subset[ , rownames(sample_clusters)]
...
```{r more_complex_heatmap3}
# Add clusters to sample_types 
sample_types <- sample_types[rownames(sample_clusters), , drop = FALSE]
sample_types$clusters <- factor(sample_clusters$clusters)
# Add sample names and convert them into factors
sample_types$Sample <- factor(rownames(sample_types), levels = rownames(sample_types))


# Create annotation plot
sample_types_annotation <- ggplot(sample_types) + scale_y_discrete(position = "right", expand = c(0,0)) +
...
# Create annotation plot
sample_clusters_annotation <- ggplot(sample_types) + scale_y_discrete(position = "right", expand = c(0,0)) +
...
mat <- mat[unfactor(taxa_clusters$Feature), unfactor(sample_types$Sample)]
...
```{r more_complex_heatmap7, fig.width=10, fig.height=8}
# Combine plots
...
# Create a phylogenetic tree from hiearchical clustering data
dendro_data <- as.phylo(taxa_hclust) 

# Plot dendogram
taxa_dendrogram <- ggtree(dendro_data) + 
  theme(plot.margin=margin(0,0,0,0)) # removes margins

taxa_dendrogram
```

```{r more_complex_heatmap9}
# Create a phylogenetic tree from hiearchical clustering data
dendro_data <- as.phylo(sample_hclust)

# Plot dendogram
sample_dendrogram <- ggtree(dendro_data) + layout_dendrogram() + 
  theme(plot.margin=margin(0,0,0,0)) # removes margins
...
```{r more_complex_heatmap11, fig.width=10, fig.height=8}
# Combine plots
plot <- taxa_dendrogram + 
  row_annotation +
  sample_dendrogram + 
