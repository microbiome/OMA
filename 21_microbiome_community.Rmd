...
taxa_clusters <- taxa_clusters[order(match(rownames(taxa_clusters), labels(taxa_dendrogram))), , drop = FALSE] 
...
sample_clusters$clusters <- factor(sample_clusters$clusters)

# Order data so that it's same as in dendogram
sample_clusters <- sample_clusters[order(match(rownames(sample_clusters), labels(sample_dendrogram))), , drop = FALSE] 

# Order data based on 
tse_phylum_subset <- tse_phylum_subset[ , order(match(colnames(tse_phylum_subset), 
                                                      rownames(sample_clusters)))]
...
```{r}
sample_types <- sample_types[rownames(sample_clusters), ]
sample_types$Sample <- factor(rownames(sample_types), levels = rownames(sample_types))
sample_types <- sample_types[order(sample_types$sample_types), ]
...
dendro_data <- dendro_data(taxa_dendrogram)

taxa_dendrogram <- ggplot(segment(dendro_data)) + 
...
```{r}
dendro_data <- dendro_data(sample_dendrogram)

sample_dendrogram <- ggplot(segment(dendro_data)) + 
  geom_segment(aes(x = x, y = y, xend = xend, yend = yend)) + 
  theme_dendro() + 
  theme(plot.margin=margin(0,0,0,0)) # removes margins
...
plot <- taxa_dendrogram + 
  ...
  heatmap +
    plot_layout(design = design, guides = "collect", # Specify layout, collect legends
                ...
                # To get this work most of the times, you can adjust all sizes to be 1, i.e. equal, 
                # but then the gaps between plots are larger.
                widths = c(0.25, 0.15, 1, 1, 1),
                heights = c(0.25, 0.2, 0.25, 1, 1))
