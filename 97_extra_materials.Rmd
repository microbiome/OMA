...
Here we present two possible uses of the `adonis2` function which performs PERMANOVA. The optional argument `by` has an effect on the statistical outcome, so the two possibilities are compared in this section.
...
formulas <- apply(var_perm, 1, function(row) reduce(row, function(x, y) paste(x, "+", y)))
...
for (row_idx in 1:nrow(var_perm)) {
  ...
  tmp_formula <- reduce(var_perm[row_idx, ], function(x, y) paste(x, "+", y))
...
The following table displays the p-values for the three variables ClinicalStatus, Gender and Nationality obtained by PERMANOVA with `adonis2`. As you can see, p-values remain identical when `by = "margin"`, but they change with the order of the variables in the formula when `by = terms` (default).

```{r message = FALSE, warning = FALSE}
df <- terms_df %>%
  inner_join(margin_df, by = "Formula", suffix = c(" (terms)", " (margin)"))
