...
This section will have a cheatsheet for translating common functions in Phyloseq to TreeSE/mia
...
After learning how to access various data types from TreeSE, let's see how creating TreeSE objects compares to creating Phyloseq objects. We will use the vanilla data frames we created from the Phyloseq object to demonstrate making both types od data objects.
...
Since TreeSEs can hold many different versions of the OTU table, most commonly either relative abundances or counts, we will need to give our assay a name and list the different types of assays we have. In this example we only have one item 'counts' in the list.
...
Here we'll show an exampe of how to add relative abundances and CLR normalized OTU tables to your TSE assays.
...
```{r}
# We can also add an other assay that holds the relative abundance normalized OTU table
...
PHY_relab  = transform_sample_counts(PHY, function(x) x / sum(x) )
...
# With phyloseq tiy would need to have a third phyloseq object
...
PHY_nomock_bacteria # Inspect, now we have 19008 taxa (only bacteria), before it was 19216
TSE_nomock_bacteria 
...
df <- estimate_richness(PHY, measures = "Shannon") # Phyloseq uses estimate_richness 

head(df) # Inspect

# Add Shannon alpha diversity to the sample data to keep it safe with other sample level data
...
We can calculate PcoA with Bray-Curtis distances in Phyloseq using the ordiate command. This 
The beta diversity calculation in mia  outputs a TreeSE with a new type of data, reduced dimensions or reducedDim. Here we will use the scater package that runs the PcoA with runMDS. (PcoA and MDS mean the same thing)
...
# For the ordination using mia we use runMDDS from the scater package and implement the vegan bray distance calculation
TSE <- runMDS(TSE,
              FUN = vegan::vegdist,
              method = "bray",
...
It is also easy to plot the ordination stored in reducedDim in the TSE using the plotReducedDim. We can first check what the name of the Bray-Curtis MDS/PCoA was.
```{r}
...
# The sign is given arbitrarily so we can change it to match the plot_ordination
...
This data object can again be conveniently stored in a TSE object under altExp.
...
Tax glom removes the taxa which have not been assigned to the level given in taxrank by default. NArm = TRUE so we will add the onRankOnly = TRUE which is equivalent to the mergeFeaturesByRank.
...
```{r}
library(knitr)
df <- data.frame(Functionality=c("Access sample data", "Access tax table", "Access OTU table", "Build data object", "Calculate alpha diversity", "Calculate beta diversity", "Plot ordination", "Subset taxa", "Subset samples", "Aggromerate taxa"), phyloseq=c("sample_data()", "tax_table()", "otu_table()", "phyloseq()", "estimate_richness()", "ordinate()", "plot_ordination()", "subset_taxa()", "subset_samples()", "tax_glom()"), miaTreeSE = c("Index columns", "Index rows", "assays()", "TreeSummarizedExperiment()", "estimateDiversity()", "runMDS()", "plotReducedDim()", "Index rows", "Index columns", "mergeFeaturesByRank()"))

df2 <- data.frame(Data_type=c("OTU table", "Taxonomy table", "Sample data table"),phyloseq=c("otu_table", "tax_table", "sample_data"), TreeSE=c("assay","rowData","colData"))
kable(df)
kable(df2)
```
