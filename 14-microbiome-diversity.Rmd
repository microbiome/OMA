...
Richness is gives the number of features present within a community and can be calculated with `estimateRichness`. Each of the estimate diversity/richness/evenness/dominance functions adds the calculated measure(s) to the `colData` of the `SummarizedExperiment` under the given column `name`.  

```{r}
se <- estimateRichness(se, 
                       abund_values = "counts", 
                       name="richness")

head(colData(se)$richness)
```
...
The resulting `data.frame` consists of two columns, `PD` and `SR`. The column `PD` has phylogenetic diversity values and `SR` is species richness (same as observed richness). We will add these values to the `colData` of the `SummarizedExperiment`.  
...
plots <- lapply(c("richness", "shannon","simpson", "relative", "PD"),
                plotColData,
                object = se,
                x = "SampleType",
                colour_by = "SampleType")
...
Alternatively, all available indices within `mia` can be calculated by default using `estimateAlphaDiversity`...
