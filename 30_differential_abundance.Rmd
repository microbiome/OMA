...
# Select two locations for simplicity
tse0 <- tse0[ , tse0$Geographical_location %in% c("Pune", "Nashik")]
# Convert location from string to factor
tse0$Geographical_location <- factor(tse0$Geographical_location)

# Show number of observations by location
table(colData(tse0)$Geographical_location) %>%
  knitr::kable()
```
...
altExp(tse, "genus") <- agglomerateByRank(tse,
                                          rank = "genus")
...
out <- ancombc2(data = altExp(tse, "genus")[prevalent.genera, ],
                assay_name = "counts",
...
maaslin2_out <- Maaslin2(input_data = t(assay(tse)),
                         input_metadata = data.frame(colData(tse)),
                         output = "DAA example",
                         transform = "AST",
