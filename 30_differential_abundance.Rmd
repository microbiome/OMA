...
# this chunk will only be run once and can be deleted thereafter on this 
# server (must be run if book will be run on another server though)
if(!"MicrobiomeStat" %in% installed.packages()[, "Package"]) {
...
We recommend to have a look at @nearingMicrobiomeDifferentialAbundance2022 who compared all these listed methods across 38
...
Unfortunately, as @nearingMicrobiomeDifferentialAbundance2022 point out, they
...
and your theory, you must determine how to specify model exactly. E.g., there
might be confounding factors that might drive (the absence of) differences
between the shown groups that we ignore here for simplicity. However, we will
show how you could include covariates in those models. Furthermore, we picked a
dataset that merely has microbial abundances in a TSE object as well as a
grouping variable in the sample data. We simplify the analysis by only
including 2 of the 3 groups. 
...
@nearingMicrobiomeDifferentialAbundance2022 found that applying a 10% threshold
...
independent evaluation by @nearingMicrobiomeDifferentialAbundance2022.  Please
also have a look at the more extensive [vignette](https://bioconductor.org/packages/release/bioc/vignettes/ALDEx2/inst/doc/ALDEx2_vignette.html) 
...
size of 1. See @gloorDisplayingVariationLarge2016 to learn more about these plots.
...
[@nearingMicrobiomeDifferentialAbundance2022]. 
...
Lastly, we cover linear models for differential abundance analysis of microbiome compositional data (@zhouLinDALinearModels2022). This tool is very similar to ANCOMBC with few differences: 1) LinDA correct for the compositional bias differently 
