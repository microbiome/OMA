...
Another way to agglomerate the data is to do clustering on the taxa. To do so,
...
Then we do the CLR transform followed by the clustering.

```{r taxa_clustering}
# The result of the CLR transform is stored in the assay clr
tse <- transformCounts(tse, assay.type = "counts", method = "clr", 
                       pseudocount = 1, MARGIN = "features")

# We do the clustering on the features of the clr assay 
tse <- cluster(tse, assay.type = "clr", HclustParam(method = "ward.D2"), 
               MARGIN = "features", clust.col = "hclust")
```

The result is stored in the `rowData` column specified with the `clust.col` 
parameter.

```{r taxa_clustering_result}
# Checking the clusters
clusters <- rowData(tse)$hclust
head(clusters, 10)
```
...
library(ggplot2)
ggplot(data.frame(clusters = clusters), aes(x = clusters)) + 
    geom_histogram(stat = "count") + 
    labs(title = "Histogram of clusters occurrence amongst the features",
       x = "Clusters", y = "Feature count") +
...
# Merging the rows by cluster
tse_merged <- mergeRows(tse, clusters)
tse_merged
```
