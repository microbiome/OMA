...
1. Import the mia and ALDEx2 packages, load peerj13075 with `data` and store it
   into a variable named `tse`.
2. Agglomerate the TreeSE by genus and filter by a prevalence of 10%. You can
   perform both operations in one go with `subsetByPrevalentTaxa` by specifying
...
4. Feed `x` to the functions `aldex.ttest` to erform t-test and to `aldex.effect`
...
### Controlling for confounders {#control-daa-confounders}

1. Import the mia and MicrobiomeStat packages, load peerj13075 with `data` and
   store it into a variable named `tse`.
2. Agglomerate the TreeSE by genus and filter by a prevalence of 10%. You can
   perform both operations in one go with `subsetByPrevalentTaxa` by specifying
   the `rank` and `prevalence` arguments.
...
### Comparing methods {#compare-daa-methods}
...
1. Import the mia and GUniFrac packages, load peerj13075 with `data` and
   store it into a variable named `tse`.
2. Agglomerate the TreeSE by genus and filter by a prevalence of 10%. You can
   perform both operations in one go with `subsetByPrevalentTaxa` by specifying
   the `rank` and `prevalence` arguments.
