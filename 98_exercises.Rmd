...
Here we learn some explorative methods and apply to the HintikkaXOData. If you
solved the previous exercise, you can use the MAE you created and skip the first
step.

1. Store the HintikkaXOData MAE into a variable named `mae`
2. Which experiments make up the MAE? How many samples and features are contained
   by each experiment? You can check that with `dim`, `nrow` and `ncol`
3. Agglomerate the microbiota experiment by Genus and store it into a new TreeSE
   named `microbiota_genus`. How many features remain after agglomerating?
4. Add `microbiota_genus` as a new experiment to `mae`. Note that new experiments
   can be added to a MAE through simple concatenation with `c(mae, ...)`
   
So far so good. You explored the MAE, agglomerated one of its experiments and
added it to the container. Now we will perform a cross-correlation analysis
between two of the experiments in the MAE.

1. Apply a CLR transformation to the counts assay of the agglomerated microbiota
   experiment, and a log10 transformation to both the assays of the metabolite
   and biomarkers experiments. You can use the function `transformCounts` and
   don't forget to specify the assay to be transformed with the argument
   `assay.type`
2. Analyze correlations between the agglomerated microbiota and the biomarkers
...
3. What does the output look like? By default, correlation is measured in terms
   of Kendall tau coefficients. Repeat point 2, but this time change the method
...
Congratulations! You performed a cross-correlation analysis on a MAE data
container. Feel free to further test your skills with the following tips:

* Perform cross-correlation analysis between the remaining experiments
  (microbiota vs metabolites and metabolites vs biomarkers)
* Focus on the significant correlations between taxa and biomarkers and
  predict those biomarkers from taxa by means of linear regression
* Move further onto multi-omics factor analysis (\@ref(mofa)).
