...
```{r load_bluster}
# Load dependencies (remove github dependency in the next bioc release)
detach("package:bluster", unload = TRUE)
devtools::install_github("LTLA:bluster", force = TRUE)
library(bluster)
...
The main focus here will be how to use mia's `cluster` function to do the 
clustering.
...
The `cluster` function's parameters allow to choose which algorithm to use, 
whether we want to cluster on samples or taxa (amongst other things). Here 
for example, we cluster using the hierarchical clustering and on the samples 
(`MARGIN` parameter).

```{r bluster_sample_hclust1}
# Simple use of the hierarchical clustering which sets the cut height to
# half the dendrogram height.
tse <- cluster(tse, assay.type = "relabundance", MARGIN = "samples", HclustParam())

# Check the result contained in the clusters part of colData. To change the
# name clusters, you can change the parameter clust.col
...
Check dedicated part to see other examples.
...
# Find clusters
cutree(hclust_data, k = 2)

# Making colors for 15 clusters
...
Based on the result of silhouette analysis, we choose 2 to be the number of clusters
...
```{r dmm2}
tse_dmm <- cluster(tse, name = "DMM", DmmParam(k = 1:7, type = "laplace"), MARGIN = "samples", full = TRUE)
```

```{r}
# The dmm info is stored in the metadata (the label is the one indicated in the name parameter)
tse_dmm
```

The following operation returns a list of DMN objects for a closer investigation.
...
```{r}
dmn_group <- calculateDMNgroup(tse_dmm,
...
ClustAndPlot <- function(x) {
  # Adding the clustering data from the short random walks algorithm to the TSE
