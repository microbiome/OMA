...
## Introduction to bluster package and wrapper
...
*bluster*'s main function `clusterRows`, used to do the clustering can be 
used via mia's `cluster` function.
...
tse <- cluster(tse, MARGIN = "samples", HclustParam())
...
tse <- cluster(tse, HclustParam(method = "complete"), full = TRUE)


# Information is stored in the metadata under the name "clusters". That
# can be changed using the parameter "name"
hclust_data <- metadata(tseb)$clusters$hclust
