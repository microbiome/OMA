...
it. It's also possible to use `transformCounts` to transform the data 
for the clustering.
```{r load-bluster}
# Load dependencies
if (!require(bluster)) {
...
The clustering can be done on the features or the samples.

### Samples clustering

To cluster the samples, we simply need to transpose the assay so that 
the samples are in rows.
```{r sample-clust-prep}
assay <- t(assay(tse, "relabundance"))
```

Then, to do the clustering, use the `clusterRows` function. Depending 
on its parameters, it will perform a different algorithm. Here, we'll 
perform a hierarchical clustering.
```{r bluster-sample-hclust1}
...
```{r bluster-sample-plot}
# Add the MDS dimensions for plotting
tse <- runMDS(tse,
  assay.type = "relabundance", FUN = vegan::vegdist,
  method = "bray"
)
...
```{r bluster-sample-hclust2}
# More complex use of the algorithm with another method and cut height
hclust.out <- clusterRows(assay, HclustParam(method = "complete", cut.dynamic = TRUE))
hclust.out
```

Sometimes we need more information from the clustering, for example to 
plot the dendogram. In that case we use the `full` parameter.
```{r bluster-sample-dendo}
...
It's also possible to do clustering for the taxa. We'll do once again the 
...
tse <- transformCounts(tse,
  assay.type = "relabundance",
  method = "clr",
  pseudocount = min(assay(tse, "relabundance")[assay(tse, "relabundance") > 0])
)

# Cluster the taxa using hierarchical clustering
hclust.out <- clusterRows(assay(tse, "clr"), HclustParam())

# Put the cluster data in rowData
rowData(tse)$clusters <- hclust.out

# Merge the rows
mergeRows(tse, rowData(tse)$clusters)
```
