...
The data can be clustered either based on features or samples. Hence, depending on the analysis goal the data might require transformation. The examples below are focused on sample clustering. To learn about feature clustering, check out chapter 6.3.
...
# The result contained in the clusters column of colData

# The number of samples included in each cluster
summary(colData(tse)$clusters)
```
...
```{r dmm7}
head(colData(tse_dmm)$clusters, 10)
```
...
```{r dmm8}
head(metadata(tse_dmm)$DMM$prob, 10) 
# Can be accessed from in best model infromation as well using:
# metadata(tse_dmm)$DMM$dmm[2][[1]]@group
```
...
clusters <- round(metadata(tse_dmm)$DMM$prob, 1)
clusters <- as.data.frame(cbind(clusters, levels(colData(tse_dmm)$SampleType)[colData(tse_dmm)$SampleType])) # add sample type information
...
```{r dmm10, fig.width=8, fig.height=4}
# Get the estimates on how much each phyla contributes on each cluster
best_model <- metadata(tse_dmm)$DMM$dmm[2]
...
for (i in 1:2) {
  ...
  p <- ggplot(head(drivers, 10), aes(x = reorder(head(phyla, 10), + head(drivers[[i]], 10)), y = head(drivers[[i]], 10))) +
          ...
          coord_flip() + labs(title = paste("Top phyla in a group", i)) +
          theme_light(base_size = 15) + xlab("") + ylab("") + ylim(0, 7)
  print(p)
}
```


We use `calculateDMNgroup` function to have an overview of the best model. The function groups samples by sample type and returns DMNGroup object that contains a summary.
...
In this section we show how to calculate principal coordinates for clr transformed abundance data. We use Aitchison distance as a distance metrics that calculates Euclidean distances for clr transformed compositions. In the visualization section, the samples are coloured based on their DMM clusters to demonstrate the distances between samples included in each DMM cluster.

```{r dmm12}
# pseudocount is added, because data contains zeros
