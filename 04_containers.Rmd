...
[@tengeler2020gut]. The dataset is briefly presented in
...
The dataset consists of 3 files, which can be fetched or downloaded from
...
# Read in biom file
biom_obj <- biomformat::read_biom(biom_file_path)
...
```{r}
# CSV file without headers and rownames in the first column
sample_meta <- read.table(sample_meta_file_path,
...
Next, we add a phylogenetic tree to the rowData slot. Such feature is available only in TreeSE objects. Similarly, Trees specifying the sample hierarchy can be stored in the colTree slot.
...
Finally, we save the complete TreeSE object as an RDS file, so that we can
readily import and use it in the future.
