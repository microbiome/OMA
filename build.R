...
rm.fs <- c("R/dada2_workflow.R",
           "R/40_machine_learning.R"
	   )
fsr <- setdiff(fsr, rm.fs) # Fix those files later
...
# These are also useful but need separate install
library(mikropml)
library(GUniFrac)
library(gridExtra)
library(MLeval)
...
# Build the book - must be run separately
