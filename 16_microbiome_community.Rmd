...
if(!require(microbiomeDataSets)){
    install.packages("microbiomeDataSets")
}
# Imports the data
...
taxa <- as.character(
  unique(correlation_table[correlation_table[["p.adj"]] < 0.05 &
                             abs(correlation_table[["Correlation"]]) > 0, "X1"]))
# Gets taxa that has at least one statistically significant correlation
lipids <- as.character(
  unique(correlation_table[correlation_table[["p.adj"]] < 0.05 &
                             abs(correlation_table[["Correlation"]]) > 0, "X2"]))
