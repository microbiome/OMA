...
We compare two algorithms results, "short random walks"[@Pons2006] versus "louvain"[@Blondel2008]. 
Graph is constructed using different k values (the number of nearest neighbors to consider during graph construction) 
using the clr assay data. Then plotting the communities using the earlier PCoA ordination.
The results obtained using "short random walks":
...
k <- c(2,3,5,10)
ClustAndPlot2 <- function(x) {
  graph_clusters <- clusterRows(t(assays(tse)$clr), NNGraphParam(k=x,cluster.fun="louvain"))
euclidean_pcoa_df$communities <- graph_clusters
ggplot(data = euclidean_pcoa_df, 
                             aes(x=pcoa1, y=pcoa2,
                                 color = communities)) +
