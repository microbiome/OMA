...
using the clr assay data. Then plotting the communities using UMAP ordination as a visual exploration aid.
...
# Calculating the log transformation required by UMAP ordination 
tse <- transformCounts(tse, method = "log10", pseudocount = 1)
# Performing and storing UMAP
tse <- runUMAP(tse, name="UMAP", exprs_values="log10")
...
for each case presented earlier:
...
ClustDiagPlot <- function(x) {
  ...
  sil <- approxSilhouette(t(assays(tse)$clr), graph_clusters)
