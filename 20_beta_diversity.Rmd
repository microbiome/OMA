...
p <- plotReducedDim(tse, "MDS_BC", colour_by = "SampleType")

# Add explained variance for each axis
e <- attr(reducedDim(tse, "MDS_BC"), "eig");
rel_eig <- e/sum(e[e>0])		  
...
plots <- lapply(c("MDS_BC","MDS_euclidean","NMDS_BC","NMDS_euclidean"),
                plotReducedDim, object = tse, colour_by = "SampleType")
...
```{r}
tse <- runMDS(tse, FUN = calculateUniFrac, name = "UniFrac",
              tree = rowTree(tse),
              ntop = nrow(tse),
             exprs_values = "counts")
```
...
plotReducedDim(tse, "UniFrac", colour_by = "SampleType")
```
...
plotReducedDim(tse, "TSNE", colour_by = "SampleType", ncomponents = c(1:3))
```
...
```{r}
tse_lahti <- relAbundanceCounts(tse_lahti)
tse_lahti <- runNMDS(tse_lahti, FUN = vegan::vegdist, name = "BC", nmdsFUN = "monoMDS",
                     exprs_values = "relabundance",
                     keep_dist = TRUE)
```

```{r}
plotReducedDim(tse_lahti, "BC", colour_by = "group")
```
...
```{r}
library(vegan)
permanova <- vegan::adonis(t(assay(tse_lahti,"relabundance")) ~ group,
                           data = colData(tse_lahti),
                           permutations = 9999)
...
```{r}
anova(vegan::betadisper(attr(reducedDim(tse_lahti,"BC"),"dist"),
                        colData(tse_lahti)$group))
```
