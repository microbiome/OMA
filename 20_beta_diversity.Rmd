...
A few combinations of beta diversity metrics and assay types are conventionally
used. For instance, Bray-Curtis dissimilarity and Aitchison distance are often applied to the relative abundance and the clr assays, respectively. Besides
**beta diversity metric** and **assay type**, the **PCoA algorithm** is also a variable that should be considered. Below, we show how the choice of these three
factors can affect the resulting lower-dimensional data.

```{r mds-nmds-comparison, warning=FALSE, message=FALSE}
...
plots <- lapply(c("MDS_bray", "NMDS_aitchison",
