...
.manipulate_bc_data <- function(bc_clusters, assay, row_col){
  ...
  colnames(bc_clusters) <- paste("cluster_", 1:ncol(bc_clusters))
  return(bc_clusters)
}
```
...
row_biclust_info <- .sum_mean_median_var(mae[[3]], "signals", bicluster_columns)

col_biclust_info 
row_biclust_info 
```

Now we can create a scatter plot to check if clusters really differs from each others.

```{r biclust_7}
df <- cbind(col_biclust_info[, "sum"], row_biclust_info[, "sum"])
colnames(df) <- c("sum_col", "sum_row")
df <- cbind(df, data.frame(variance = col_biclust_info$variance + row_biclust_info$variance),
            cluster = rownames(col_biclust_info))

ggplot(df)  +
    geom_point(aes(x = sum_row, y = sum_col, color = cluster, size = variance))
...
```{r biclust_9, fig.width=10, fig.height=8}
