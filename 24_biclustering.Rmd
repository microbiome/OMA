...
```{r biclust_5}
# Function for obtaining sum, mean, median, and mean variance
.sum_mean_median_var <- function(tse, abund_values = "counts", clusters){
  ...
  df <- data.frame(lapply(clusters, function(x){
    ...
    sum <- sum(assay, na.rm = T)
    mean <- mean(assay, na.rm = T)
    median <- median(assay, na.rm = T)
    var <- mean(rowVars(assay, na.rm = T))
    return(c(sum, mean, median, var))
  }))
  # Add right rownames
  rownames(df) <- c("sum", "mean", "median", "variance")
  # Transpose
  df <- t(df)
...
# Calculate info
col_biclust_info <- .sum_mean_median_var(mae[[1]], "clr", bicluster_rows)
row_biclust_info <- .sum_mean_median_var(mae[[2]], "nmr", bicluster_columns)

col_biclust_info 
row_biclust_info 
```

Now we can create a scatter plot to check if clusters differs from each others.

```{r biclust_6}
# Combines medians into one data.frame
df <- cbind(col_biclust_info[, "median"], row_biclust_info[, "median"])
colnames(df) <- c("median_col", "median_row")
# Adds sum of variance and clusters
df <- cbind(df, data.frame(variance = col_biclust_info$variance + row_biclust_info$variance),
            cluster = rownames(col_biclust_info))

ggplot(df)  +
    geom_point(aes(x = median_row, y = median_col, color = cluster, size = variance))
...
```{r biclust_8, fig.width=10, fig.height=8}
