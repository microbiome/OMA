...
`r BiocStyle::Biocpkg("mia")` with example code.
...
Start by loading data as a `r BiocStyle::Biocpkg("phyloseq")` object `phy` and
as `TreeSE` object `tse`.
...
There are slightly different names for these tables in
`r BiocStyle::Biocpkg("phyloseq")` and `TreeSE`, but they can be retrieved
from the `r BiocStyle::Biocpkg("phyloseq")` and `TreeSE` containers in
analogous ways.

**Accessing the table of taxonomic names: tax_table = rowData**

`r BiocStyle::Biocpkg("phyloseq")` and `TreeSE` objects' taxonomy tables can
be accessed with `tax_table()` and `rowData()` commands.
...
OTU tables can be accessed with `otu_table()` and `assay()` commands. The assay 
can also hold other types of information like taxa abundances from shotgun
...
creating `TreeSE` objects compares to creating
`r BiocStyle::Biocpkg("phyloseq")` objects. We will use the vanilla data frames
we created from the `r BiocStyle::Biocpkg("phyloseq")` object to demonstrate
making both types of data objects. These are identical to the equivalent `tse`
data frames but for demonstration we will use ones created from `phy`.
...
These are all normal data frames which could come from upstream
...
a `r BiocStyle::Biocpkg("phyloseq")` object and how `assay` in `TreeSE`
compares to `otu_table` in `r BiocStyle::Biocpkg("phyloseq")`.
...
Since `TreeSE`s can hold many different versions of the OTU table, most
commonly either relative abundances or counts, we will need to give our `assay`
(which corresponds to `otu_table` in `r BiocStyle::Biocpkg("phyloseq")`) a
name and list the different types of assays or transformations we have. In this
...
Adding the assays as a list might seem inconvenient if you only have one type
of OTU table (`counts` in our example), but let's see why it is actually very
...
Next let's learn how to subset samples. In `r BiocStyle::Biocpkg("phyloseq")`
we use `subset_samples()` command, but since the sample data is stored in
columns in the `TreeSE`, we can access it by indexing columns.
...
Taxa are stored in rows in `TreeSE` and the `TreeSE` equivalent to
`subset_taxa` is indexing rows.
...
`r BiocStyle::Biocpkg("mia")` compared to `r BiocStyle::Biocpkg("phyloseq")`
package.

The `r BiocStyle::Biocpkg("mia")` command  `estimateDiversity()` will return a
`TreeSE` and the results are stored in `colData`, unlike the
`r BiocStyle::Biocpkg("phyloseq")` command that outputs a data frame with
just the diversity estimates.

In `r BiocStyle::Biocpkg("phyloseq")` you would need to add the alpha
diversity separately to your sample data to keep it safe with the other
sample level data.
...
We can calculate PCoA with Bray-Curtis distances in
`r BiocStyle::Biocpkg("phyloseq")` using the `ordinate()` command. The beta
diversity calculation in `r BiocStyle::Biocpkg("mia")` outputs a `TreeSE`
with a new type of data, reduced dimensions or `reducedDim`. 

Here we will use the `r BiocStyle::Biocpkg("scater")` package that runs the
PCoA with `addMDS()`. (PCoA and MDS mean the same thing)

In `r BiocStyle::Biocpkg("phyloseq")` you would again need to add the
dimensions to the sample data if you want to keep them safe with other metadata.
...
`r BiocStyle::Biocpkg("phyloseq")` has it's own plotting fuction for
ordinations.
...
It is also easy to plot the ordination stored in `reducedDim` in the `tse`
using the `plotReducedDim()` function. We can first check what the name of the
Bray-Curtis MDS/PCoA was in case we forgot.
...
This family level data object can again be conveniently stored in a `tse`
object under `altExp`.
