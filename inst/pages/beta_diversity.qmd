...
```{r "MDS plot based on the Bray-Curtis distances on the GlobalPattern dataset."}
...
ntop <- 5
tse <- tse[head(rev(order(rowSds(assay(tse, "counts")))), ntop), ]
```
...
tse <- runMDS(tse, 
              assay.type="counts",
              ntop = nrow(tse), 
              FUN = vegan::avgdist, # Custom beta diversity function that 
                                    # includes rarefaction
...
print(all.equal(current = cor(as.vector(reducedDim(tse, "MDS_aitchison")),
...
for (k in 1:2) {
  ...
  pcx <- reducedDim(tse, "MDS_aitchison")[,k]
  pcy <- reducedDim(tse, "MDS_aitchison_rarefied")[,k]
