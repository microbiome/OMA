...
The importance of each variable on the similarity between samples (i.e. loadings)
can be assessed from the results of PERMANOVA, automatically provided by the `addRDA`
function. It performs first dbRDA and then applies permutational test to its
results.
...
function from the vegan package. You can find more on PERMANOVA from [here](https://microbiome.github.io/OMA/docs/devel/pages/97_extra_materials.html#compare-permanova).
