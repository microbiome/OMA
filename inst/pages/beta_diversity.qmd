...
tse_sub <- runMDS(tse_sub,
  ...
  distfun = getDissimilarity, # Use vegdist beta diversity function
...
for (k in 1:2) {
  ...
  p[[k]]  <- ggplot(data = data.frame(pcx, pcy), aes(x = pcx, y = pcy)) +
        geom_point() +
        geom_abline(intercept = 0, slope = 1, color = "red") +  # Adding the line y = x
        ggtitle(paste0("PC", k, "; r=", round(r, 3))) +  # Adding the title
        theme_minimal()
}
wrap_plots(p)
```
