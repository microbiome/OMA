...
First, we will load the necessary libraries and create the `tse` object from the `GlobalPatterns` dataset.
...
Now, we'll create the NeatMap using the `ComplexHeatmap` package and the `getNeatOrder` function.
