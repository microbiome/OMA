...
calculated, and the top taxa are retrieved for the Phylum rank. Thereafter,
the barplot is visualized by ordering ranks by abundance values and
...
horizontal axis represents samples and the vertical axis represents
taxa. The color of each intersection point represents the abundance of a taxon
in a specific sample.

Here,  abundances are  first CLR  (centered log-ratio)  transformed to
remove  compositionality bias. Then, Standardize(or  'Z') transformation  is applied  to
...
absolute  abundance  levels.  After  these  initial visual  exploration
techniques, we can visualize the abundances at the Phylum level.
...
_pheatmap_ is a package that provides methods for plotting clustered heatmaps. 
...
Based on the phylogenetic tree, we decide to create three clusters. 
...
# Order data based on the phlyo tree
