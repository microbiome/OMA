...
research. In ecological literature, several distinct but related alpha
diversity indices, often referring to **richness** and **evenness** - the
number of taxonomic features and how they are distributed, respectively - are
commonly used [@Willis2019; @Whittaker1960]. The term **diversity** can be used
to collectively refer to all these indices.
...
have been associated with worse overall physical and mental health
[@Valles-Colomer2019GBMs; @vandeputte2017quantitative]. However, this principle
may not generalize to other populations, most notably in early life and in
patient cohorts [@Ma2019].

### Approaches

The majority of alpha diversity metrics are closely related, though this is not
evident from their names. @bastiaanssen2023bugs1 lay out this relationship
...
cbind.data.frame(
    ...
    opt_table_lines() |>
    tab_style(
        style = list(cell_text(v_align = "middle", align = "center")),
...
@Cassol2025 offer practical guidelines for selecting alpha diversity indices.
They classify alpha diversity into four main categories and recommend
calculating at least one metric from each category, if possible, to
...
parameter that controls the detection threshold. Species above this threshold
are considered detected. See full list of options from `help(addAlpha)`.
...
`tse` does not contain a rowTree.
:::
...
between them. Below, a visual comparison between Shannon and Faith indices is
shown with a scatter plot.
...
Let us visualize results from multiple alpha diversity measures against a given
sample grouping available in `colData` (here, sample type). These have been
readily stored in the `colData` slot, and they are thus directly available for
plotting.
...
All these different metrics provide information from different aspects of alpha
diversity, notably on richness, dominance, information, and phylogenetic
...
particularly in the freshwater creek samples — was dominated by a single
species, despite showing high observed richness.
...
### Rarefaction {#sec-rarefaction-alpha}

To control uneven sampling depths, we can do rarefaction
(see [@sec-rarefaction]). Rarefaction for alpha diversity metrics can be applied
by specifying the optional arguments `sample` and `niter` in `addAlpha()`.
`niter` controls the number of rarefaction iterations (choose a sufficiently
large value for stable estimates). `sample` sets the rarefaction depth, i.e.,
the number of reads sampled per iteration from each sample.

```{r}
#| label: alpha_rarefaction
tse <- addAlpha(
    tse,
    assay.type = "counts",
    index = "observed_richness",
    name = "richness_w_rarefaction",
    niter = 10
)
```

Next compare the results with and without rarefaction.

```{r}
#| label: compare:rarefaction
plotBoxplot(tse, col.var = "observed_richness", fill.by = "SampleType") +
    plotBoxplot(tse, col.var = "richness_w_rarefaction", fill.by = "SampleType") +
    plot_layout(guides = "collect")
```

These results illustrate that uneven sequencing depth affects the diversity
estimates: applying rarefaction changes the values compared with the
non-rarefied analysis, indicating that part of the apparent differences was
driven by sampling effort rather than biology.
...
2. Calculate the Shannon diversity index, and add the results to `colData()`.
...
this? If they correlate very strongly, why is that? What does rarefaction do
...
can find from `colData`. Can you see a difference between groups?

10. Calculate statistical significance of difference in selected alpha
diversity index between the sample groups. Do the results confirm your
observation in step 9?

Useful functions:

`utils::data()`, `mia::addAlpha()`, `miaViz::plotHistogram()`,
`mia::getCrossAssociation()`, `scater::plotColData()`,
`geom_smooth(method = "lm")`, `stats::wilcox.test()`,
