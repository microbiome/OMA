...
Then, the barplot is visualized ordering rank by abundance values
...
features that have different absolute abundance levels. For more information on transformation
methods, refer to [@sec-assay-transform]. 
...
Now, we'll perform the aforementioned steps to create a NeatMap using the `sechm` package and the
...
`utils::data()`, `mia::agglomerateByRank()`, `mia::transformAssay()`, `miaViz::plotAbundance()`,
`SummarizedExperiment::colData()`, `sechm::sechm()`
