...
and top taxonomic features are retrieved for the phylum rank.
Thereafter, the barplot is visualized ordering rank by abundance values
...
Community composition can be visualized with a heatmap, where the
horizontal axis represents samples and the vertical axis the taxonomic
features. The color of each intersection point represents abundance of a
taxon in a specific sample.

Here, abundances are first transformed with CLR (centered log-ratio) to
remove compositionality bias. Then standardize transformation is applied
to the CLR-transformed data. This shifts all taxonomic features to zero
mean and unit variance, allowing visual comparison between taxonomic
features that have different absolute abundance levels. After these
rough visual exploration techniques, we can visualize the abundances at
Phylum level.
...
First, we will load the necessary libraries and load `GlobalPatterns`,
which is a `TreeSE` object. We agglomerate the data to phylum level to
explore the phyla community.

Now, we'll create the NeatMap using the `sechm` package and the
`getNeatOrder()` function.
...
`*ComplexHeatmap*` for clustered heatmaps is explained in chapter
[@sec-clustered-heatmap].
:::

::: {.callout-tip icon="false"}
## Exercises

**Goal:** After these exercises, you are able to visualize a summary of
...
4.  Visualize community with relative abundance bar plot. Which
    taxonomic features are most prevalent?

5.  Explore `colData`. Select the sample metadata variable and visualize
    the composition with a bar plot in respect to sample grouping. Does
    the community composition associate with the grouping?

6.  Normalize abundances to standardized scale, i.e., calculate
    Z-scores.

7.  Plot normalized abundances with a heatmap.
