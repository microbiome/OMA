...
Further methods of application can be found in the chapters [@sec-qc]
and [@sec-richness] and in a few [external
...
# get top orders
top_taxa <- getTopFeatures(tse_phylum,
                           top = 5,
                           assay.type = "relabundance")

# leave only names for top 10 orders and label the rest with "Other"
rowData(tse_phylum)$Phylum[!rowData(tse_phylum)$Phylum %in% top_taxa] <- "Other"

# plot composition as a bar plot
plots <- plotAbundance(tse_phylum,
                       assay.type = "relabundance",
                       rank = "Phylum",
                       order_rank_by = "abund",
                       order_sample_by = "Proteobacteria",
                       features = "SampleType")
...
tse_phylum <- tse_phylum[ , tse_phylum$SampleType %in% c("Feces", "Skin", "Tongue")]
...
sample_data$sample_types <- unfactor(colData(tse_phylum)$SampleType)
...
metadata(tse_phylum)$anno_colors$SampleType <- c(Feces = "blue", 
                                                 Skin = "red", 
                                                 Tongue = "gray")
