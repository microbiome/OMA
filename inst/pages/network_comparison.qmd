...
Here we use `r BiocStyle::Githubpkg("stefpeschel/NetCoMi")`
...
How to install `r BiocStyle::Githubpkg("stefpeschel/NetCoMi")` from GitHub is
explained in [@sec-network-learning].
...
We start with a visual comparison of the two networks using
`r BiocStyle::Githubpkg("stefpeschel/NetCoMi")`'s plot
function. The same configuration as in [@sec-network-learning] is used.

```{r}
#| label: network_plot_diet_difflay
#| fig-width: 17
#| fig-height: 8
...
legend(-0.2, -0.9,
    cex = 1.5, title = "estimated correlation:",
...
```{r}
#| label: network_plot_diet_samelay
#| fig-width: 17
#| fig-heigth: 8
...
- Differences in the edge weights can now be seen at first glance. For
example, Serratia and Citrobacter are strongly associated in the mixed diet
...
`r BiocStyle::Githubpkg("stefpeschel/NetCoMi")` also provides a plot function
to generate a differential network, where two nodes are connected if they are
differentially associated between the groups.
...
The `diffnet` object is now plotted using
`r BiocStyle::Githubpkg("stefpeschel/NetCoMi")`'s `plot()` function. 
...
@jardim2019bionetstat present a tool,
`r BiocStyle::Githubpkg("jardimViniciusC/BioNetStat")`,
for differential analysis of biological networks, which is able to compare
certain network measures between groups.

The `r BiocStyle::Githubpkg("stefpeschel/NetCoMi")` package used for
...
details. `r BiocStyle::Githubpkg("stefpeschel/NetCoMi")` offers also a
function for plotting a differential network.
