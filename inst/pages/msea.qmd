...
are randomly distributed throughout the ranked list of features (L) or
primarily found at the top or bottom. We will use the R package
`r BiocStyle::CRANpkg("gsEasy")` to conduct the MSEA test described by
@Subramanian2005.
...
available from the `r BiocStyle::Biocpkg("curatedMetagenomicData")` package
[@Lloyd-Price2019]. We aim to conduct MSEA analysis based on both taxonomic
profiles (species relative abundances) and functional profiles (pathway
relative abundances).
...
differential abundance analysis using `r BiocStyle::Biocpkg("Maaslin2")`
[@Mallick2021]. The ranked feature list from the differential abundance
analysis serves as an input for the MSEA.
...
In the next step, we will use `r BiocStyle::Biocpkg("Maaslin2")`
to fit a multivariable regression model for testing the association between
microbial species abundance versus IBD diagnosis. The analysis method we use
...
WGCNA. Here, we first create a wrapper for the MSEA analysis using the
`r BiocStyle::CRANpkg("gsEasy")` package.
...
Before running the MSEA, we also need to rank the differential analysis
results from `r BiocStyle::Biocpkg("Maaslin2")`. We use the
`r BiocStyle::Biocpkg("topGO")` package to create a list of microbe sets
from the mapping file created above.

```{r}
#| lable: debug_topGo_install_failure
#| include: false
...
```{r}
#| label: calculate_enrichment_scores
...
As before, we first run a `r BiocStyle::Biocpkg("Maaslin2")` analysis using
default settings and construct the modules using residuals from the
`r BiocStyle::Biocpkg("Maaslin2")` models.

```{r}
#| label: MSEA_preparation
...
```{r}
#| label: perform_MSEA_and_visualization
