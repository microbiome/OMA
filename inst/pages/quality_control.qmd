...
filtered_x <- rowData(x)$Phylum %in% top_phyla
...
filtered_x <- rowData(x)$Phylum %in% top_phyla_mean
...
p2 <- ggplot(df) +
    geom_histogram(
            aes(x = cum_freq), color = "black", fill = "gray", bins = 30) +
    labs(
        x = "Cummulative samples (million reads)",
        y = "Frequency") +
    theme_bw() +
    # Removes the grid
    theme(panel.grid.major = element_blank(),
