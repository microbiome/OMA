...
mae[[1]] <- subsetByPrevalent(
    mae[[1]],
    rank = "Genus", prevalence = 0.2, detection = 0.001
)
...
mae[[1]] <- transformAssay(
    mae[[1]],
    assay.type = "rclr", MARGIN = "features", method = "z",
    name = "rclr_z"
)
...
corr <- getExperimentCrossCorrelation(
    mae, 1, 2,
    assay.type1 = "rclr", assay.type2 = "log10",
...
.sum_mean_median_var <- function(tse1, tse2, assay.type1, assay.type2, clusters1, clusters2) {
    ...
    for (i in 1:ncol(clusters1)) {
        ...
        list[[i]] <- data.frame(
            sample = colnames(tse1), sum1, sum2, mean1,
...
for (i in seq_along(df)) {
    pics[[i]] <- ggplot(df[[i]]) +
        ...
        labs(
            title = paste0("Cluster ", i), x = "Taxa (rclr median)",
