...
Now we can read in the biom file and convert it into a `TreeSE` object. In addition, we retrieve the rank names from the prefixes of the feature names and then remove them with the `rank.from.prefix` and `taxa.prefix.rm` optional arguments.
...
col.data <- read.csv(sample_meta_file_path,
                        sep = ",", row.names = 1)
...
colData(tse) <- DataFrame(col.data)
```
