...
The following example demonstrates how to identify the most prevalent taxa for
each sample type. Given that some samples originate from different origin, we
expect differences between sample types. While `getPrevalent()` can be used to
determine the most prevalent taxa across the entire dataset, this approach would
