...
The Faith index is returned by the function `addAlpha` with index = "faith_diversity"

```{r phylo-div-1}
tse <- mia::addAlpha(tse,                    
                    assay.type = "counts",
                    index = "faith_diversity")
head(tse$faith_diversity)
```
...
```{r phylo-div-2}
plots <- lapply(c("shannon", "faith_diversity"),
                plotColData,
...
Alternatively, the phylogenetic diversity can be calculated by `mia::addAlpha` with index = c("shannon", "faith_diversity"). This is a faster re-implementation of   
...
tse <- mia::addAlpha(tse, 
                    assay.type = "counts",
                    index = c("shannon", "faith_diversity"))
```
...
ggplot(colData(tse), aes(x=shannon, y=faith_diversity)) +
  geom_point() +
...
# Create the plots
plots <- lapply(c("observed_richness", "shannon", "faith_diversity"),
                plotColData,
                object = tse,
                x = "SampleType",
