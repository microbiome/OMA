...
Community diversity is a central concept in microbiome data analysis. 
...
Alpha diversities are commonly visualized using boxplots.

Here, let us compare Shannon index between sample groups. Individual
data points can be visualized with `geom_jitter`.

The `geom_signif` function tests whether these differences are
statistically significant and adds (adjusted) p-values to plot. See
also the [ggpubr package](http://www.sthda.com/english/articles/24-ggpubr-publication-ready-plots/76-add-p-values-and-significance-levels-to-ggplots/)
for additional flexibility in group-wise comparison plots and
associated p-values.
...
# Subsets the data. Takes only those samples that are from
# feces, skin, or tongue, and creates data frame from the collected data.
