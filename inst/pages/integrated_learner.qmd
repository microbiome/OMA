...
1. Import the mia package, load any of the example datasets in
`MultiAssayExperiment` format mentioned in [@sec-example-data] with `data()`,
and store it into a variable named `mae`.
...
8. Create a confusion matrix if you fit classification model. 

Useful functions:

`colData()`, `subsetByPrevalent()`, `rowSds()`, `assay()`, `rowData()`,
`transformAssay()`, `IntegratedLearner::IntegratedLearnerFromMAE()`,
`IntegratedLearner:::plot.learner()`
