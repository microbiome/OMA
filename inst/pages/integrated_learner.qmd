...
`r if(FALSE) fit$AUC.test[[2]] else NA` when considering only the
pathway abundance data in the model, and
`r if(FALSE) fit$AUC.test[[1]] else NA` for the model including only the
species abundance data. The AUC increases to
`r if(FALSE) fit$AUC.test[[4]] else NA` when using the early fusion
model and reaches `r if(FALSE) fit$AUC.test[[3]] else NA` with the late
...
`r if(FALSE) paste0(round(conf$overall[[1]]*100, 2), "%") else NA`. The
...
_`r if(FALSE) rownames(importances)[order(importances, decreasing = TRUE)][[1]] else NA`_ and
_`r if(FALSE) rownames(importances)[order(importances, decreasing = TRUE)][[2]] else NA`_ appear
