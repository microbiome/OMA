...
conducting microbiome data analysis and developing effective microbiome data science
workflows.
...
![miaverse framework.](images/FigureOverviewV2_mod.png){width="100%"}
...
microbiome analysis.
`SingleCellExperiment` class which is designed for single-cell sequencing
...
- `SingeCellExperiment` (`SCE`) [@R_SingleCellExperiment] was developed as an extension to store
copies of data to same data container.
...
p <- ggplot() +
    ...
    annotate("segment", x = c(1+0.25, 2+0.25), xend = c(2-0.25, 3-0.25), y = 1, yend = 1, arrow = arrow(length = unit(0.2, "inches"), type = "closed"))
