...
miaverse (MIcrobiome Analysis uniVERSE) is an actively developed
...
`SummarizedExperiment`-based data containers. This design choice
enhances interoperability and versatility within the broader
...
data container. `TreeSummarizedExperiment` is expanded from
`SingleCellExperiment (SCE)` by incorporating additional slots tailored
for microbiome analysis. `SingleCellExperiment` class is designed for
single-cell sequencing [@R_SingleCellExperiment]. Bioconductor offers
wide variety of tools for this field including online book Orchestrating
Single-Cell Analysis in Bioconductor (OSCA) [@Amezquita2020].
`SingleCellExperiment`, on the other hand, is further derived from
`SummarizedExperiment (SE)` class. This hierarchical relationship among
data containers means that all methods applicable to
`SingleCellExperiment` and `SummarizedExperiment` objects can also be
applied to `TreeSummarizedExperiment` objects.

-   `SummarizedExperiment` (`SE`) [@R_SummarizedExperiment] is a generic
...
-   `SingeCellExperiment` (`SCE`) [@R_SingleCellExperiment] was
    developed as an extension to store copies of data to same data
    container.

-   `TreeSummarizedExperiment` (`TreeSE`) [@R_TreeSummarizedExperiment]
    was developed as an extension to incorporate hierarchical
...
`MultiAssayExperiment` (`MAE`) [@Ramos2017] provides an organized way to
bind several different data containers together in a single object. For
example, we can bind microbiome data (in `TreeSE` container) with
metabolomic profiling data (in `SE`) container, with (partially) shared
...
The `mia` package family provides general methods for microbiome data
wrangling, analysis and visualization.

-   [mia](https://microbiome.github.io/mia/): Microbiome analysis tools
    [@R_mia]
-   [miaViz](https://microbiome.github.io/miaViz/): Microbiome analysis
    specific visualization [@Borman2024]
-   [miaSim](https://microbiome.github.io/miaSim/): Microbiome data
    simulations [@Simsek2021]
-   [miaTime](https://microbiome.github.io/miaTime/): Microbiome time
    series analysis [@Lahti2021]
-   [miaDash](https://microbiome.github.io/miaDash/): Interactive
    analysis and visualisation [@Benedetti2024miadash]
...
-   [ALDEx2](https://www.bioconductor.org/packages/release/bioc/html/ALDEx2.html)
    [@Gloor2016] for differential abundance analysis
-   [ampliseq](https://nf-co.re/ampliseq) [@straub2020] for amplicon
    sequencing analysis workflow using DADA2 and QIIME2
-   [ANCOMBC](https://bioconductor.org/packages/devel/bioc/html/ANCOMBC.html)
    [@ancombc2020] for differential abundance analysis
-   [benchdamic](https://www.bioconductor.org/packages/release/bioc/vignettes/benchdamic/inst/doc/intro.html)
    [@Calgaro2022] for benchmarking differential abundance methods
-   [curatedMetagenomicData](https://bioconductor.org/packages/release/data/experiment/html/curatedMetagenomicData.html)
    contains standardized curated human microbiome data for novel
    analyses.
-   [DESeq2](https://bioconductor.org/packages/release/bioc/html/DESeq2.html)
    [@Love2014] for differential gene expression analysis
-   [HoloFoodR](https://www.bioconductor.org/packages/release/bioc/html/HoloFoodR.html)
    for interfacing EBI HoloFood resource
-   [IntegratedLearner](http://htmlpreview.github.io/?https://github.com/himelmallick/IntegratedLearner/blob/master/vignettes/IntegratedLearner.html)
    for multiomics classification and prediction
-   [iSEEtree](https://microbiome.github.io/iSEEtree/articles/iSEEtree.html)
    [@Benedetti2025iseetree] for interactive visualisation of
    hierarchical data
-   [lefser](https://www.bioconductor.org/packages/release/bioc/html/lefser.html)
    [@Asya2024] for metagenomic biomarker discovery
-   [LimROTS](https://bioconductor.org/packages/release/bioc/html/LimROTS.html)
    for differential expression analysis for proteomics and metabolomics
-   [MaAsLin
    3](https://bioconductor.org/packages/devel/bioc/html/maaslin3.html)
    [@Nickols2024] for differential abundance analysis
-   [MGnifyR](https://www.bioconductor.org/packages/release/bioc/html/MGnifyR.html)
    for accessing and processing MGnify data in R
    -   [MGnify
        Notebooks](https://docs.mgnify.org/src/notebooks_list.html)
    -   [EMBL-EBI MGnify user guides and
        resources](https://github.com/EBI-Metagenomics/notebooks)
-   [microbiomeDatasets](https://www.bioconductor.org/packages/release/data/experiment/html/microbiomeDataSets.html)
    contains microbiome datasets loaded from Bioconductor'S
    ExperimentHub infrastructure
-   [MOFA2](https://www.bioconductor.org/packages/release/bioc/html/MOFA2.html)
    [@Arpelaguet2017] for multi-omics factor analysis
-   [NetCoMi](https://netcomi.de/) for network construction and
    Comparison
-   [radEmu](https://statdivlab.github.io/radEmu/articles/intro_radEmu_with_tse.html)
    [@clausen2025] for differential abundance analysis
-   [tidySingleCellExperiment](https://bioconductor.org/packages/release/bioc/html/tidySingleCellExperiment.html)
    and
    [tidySummarizedExperiment](https://www.bioconductor.org/packages/release/bioc/html/tidySummarizedExperiment.html)
    for data manipulation methods utilizing *tidy* paradigm
...
-   [MicrobiotaProcess](https://bioconductor.org/packages/release/bioc/html/MicrobiotaProcess.html)
    [@Xu2023] framework for the "tidy" analysis of microbiome and other
    ecological data
-   [microeco](https://chiliubio.github.io/microeco_tutorial/)
    [@Liu2020] microbiome analysis framework
-   [microSTASIS](https://www.bioconductor.org/packages/release/bioc/html/microSTASIS.html)
    [@Sanchez2022] for microbiota stability assessment via iterative
    clustering
-   [philr](http://bioconductor.org/packages/devel/bioc/html/philr.html)
    [@Silverman2017] phylogeny-aware phILR transformation
-   [PLSDAbatch](https://www.bioconductor.org/packages/release/bioc/html/PLSDAbatch.html)
    [@Wang2023] for batch effect correction
-   [treeclimbR](https://github.com/csoneson/treeclimbR) [@Hang2021] for
    finding optimal signal levels in a tree
-   [vegan](https://vegandevs.github.io/vegan/) [@R_vegan] for community
    ecologists
-   [Tools for Microbiome
    ...
    minor modifications. The phyloseq-based tools can be used by
    converting the TreeSE data into phyloseq with `convertToPhyloseq()`
    (see [@sec-convert]).
...
We also provide the Microbiome Analysis Dashboard (miaDash), a web app
to interactively explore microbiome data through an intuitive Graphical
User Interface (GUI). The app features a large part of the mia
...
-   `TreeSummarizedExperiment` is derived from `SummarizedExperiment`
    class.
-   `miaverse` is based on `TreeSummarizedExperiment` data container.
-   We can borrow methods from packages utilizing `SingleCellExperiment`
...
2.  Go and explore [this CSV
    file](https://raw.githubusercontent.com/microbiome/OMA/devel/oma_packages/oma_packages.csv).
    Which packages have you already used? Which packages are new to you?

3.  Search each package from internet. What tools does each package
    offer?

4.  Go to [the discussion
    forum](https://github.com/microbiome/OMA/discussions). Here you can
    get help and ask questions if you come up with one.
