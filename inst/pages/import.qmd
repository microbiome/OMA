...
from [this
repository](https://github.com/microbiome/data/tree/main/Tengeler2020):
...
effectively replacing the existing, empty `colData`. The method **does
not check sample names**, which can lead to mistakes. Therefore, you
...
files](https://github.com/microbiome/OMA/tree/master/data) and construct
your own CSV files accordingly.
...
-   Row data (`rowdata_taxa.csv`): data table (features x info); first
    ...
    taxonomic levels. Ideally, the feature IDs (row names) match
    one-to-one with the abundance table row names.
...
-   Column data (`coldata.csv`): data table (samples x info); first
    ...
    subject age, health etc). Ideally, the sample IDs match one-to-one
    with the abundance table column names.
...
-   `rowdata` (`tax`): `DataFrame`, with feature IDs as rownames. If
    this is a `data.frame` you can use the function `DataFrame()` to
    change the format. Column names are free but in microbiome analysis
    they usually refer to taxonomic ranks. The rownames in rowdata
...
-   [mia
    reference](https://microbiome.github.io/mia/reference/index.html)
-   [miaViz
    reference](https://microbiome.github.io/miaViz/reference/index.html)
-   [miaTime
    reference](https://microbiome.github.io/miaTime/reference/index.html)
...
Check the [manual pages of each
...
[original
website](https://github.com/seandavi/MicroBioMap#microbiome-compendium)
...
database. For more use examples in R/Bioconductor, see the [MicroBioMap
vignette](https://seandavi.github.io/MicroBioMap/articles/overview.html).
...
Download the CSV files from [this
...
-   `rowdata` → contains metadata for features (e.g., taxonomic
    information)
-   `coldata` → contains metadata for samples (e.g., sample
    descriptions)
...
1.  Download [this BIOM
    file](https://raw.githubusercontent.com/microbiome/OMA/refs/heads/devel/inst/extdata/Aggregated_humanization2.biom).
...
`utils::read.csv()`, `MultiAssayExperiment::DataFrame()`, `TreeSummarizedExperiment::TreeSummarizedExperiment()`,
`base::matrix()`, `BiocGenerics::rownames()`, `BiocGenerics::colnames()`, `S4Vectors::SimpleList()`,
