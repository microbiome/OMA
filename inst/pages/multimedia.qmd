...
tse[["Time_point"]] <- ifelse(
  tse[["visit_number"]] == 1, "T0", "T1"
)
...
rownames(se_relative) <- rownames(tse)
rownames(rowData(se_relative)) <- rownames(tse)  # keep rowData aligned too
...
se_pathway <- as(tse, "SummarizedExperiment")
rownames(se_pathway) <- rownames(tse)
rownames(rowData(se_pathway)) <- rownames(tse)  # keep rowData aligned too
