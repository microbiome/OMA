...
diss <- as.matrix(getDissimilarity(
    tse, method = "bray", assay.type = "relabundance", na.rm = TRUE
))
...
tse$treatment <- factor(
    ifelse(tse$visit_number == 1, "T0", "T1"), levels = c("T0", "T1")
)
tse <- transformAssay(
...
tse <- transformAssay(
    tse, assay.type = "pathway_abundance", method = "standardize", name = "scaled"
)
