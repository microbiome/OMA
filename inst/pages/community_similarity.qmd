...
obtained from the sequencing process. A variation in sequencing depth across
the samples of a study may impact the calculation of alpha and beta diversity
metrics [@Schloss2023]. It is common to find significant variation in
sequencing depth between samples. For instance, the samples of the
*TreeSummarizedExperiment* dataset `GlobalPatterns` show up to a 40-fold
difference in the number of metagenomic reads.
...
reads counts using subsampling. First, the user chooses the rarefaction depth
...
drawn from the samples left to get subsamples fitting the rarefaction depth.
...
You can find more on PERMANOVA from
