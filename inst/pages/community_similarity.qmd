...
variety of dissimilarity, or beta diversity, indices. This method is available via
`mia::getRDA()`, which calls `vegan::dbrda()`. By default, this uses Euclidean
distances, which is equivalent to the ordinary RDA. However, the dbRDA method
(`mia::getRDA()`) allows the use of other dissimilarity indices as well.
...
* PERMANOVA: (e.g. from `vegan::adonis2()`) can be used to assess
  ...
  groups. However, retrieving the loadings and components is more tricky.
