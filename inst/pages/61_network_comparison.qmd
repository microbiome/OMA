...
This chapter assumes that you are already familiar with how to construct and analyze a single microbial network, which is explained in [@sec-network-learning].
...
```{r split_data}
table(tse$Diet)
tse_list <- splitOn(tse, f = "Diet", use.names = TRUE, MARGIN = 2)
```
...
We start with a visual comparison of the two networks using NetCoMi's plot function. The same configuration as in [@sec-network-learning] is used.
...
As in [@sec-network-learning], we also generate a network plot using phylum names to color the nodes and mclr-transformed abundances to scale node sizes.
