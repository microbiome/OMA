...
networks_diet_file  <- system.file("extdata", "networks_diet.RData", package = "OMA")
load(networks_diet_file)
```

This chapter assumes that you are already familiar with how to construct and analyze a single microbial network, which is explained in Chapter [@sec-network-learning].
...
See Section [@sec-netcomp-methods] for further details on these two approaches and the methods used in this chapter.
...
We perform the same data preprocessing steps as in chapter [@sec-network-learning].
...
We start with a visual comparison of the two networks using NetCoMi's plot function. The same configuration as in chapter [@sec-network-learning] is used.
...
As in Chapter [@sec-network-learning], we also generate a network plot using phylum names to color the nodes and mclr-transformed abundances to scale node sizes.
