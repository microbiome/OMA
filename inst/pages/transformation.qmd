 ...
 * **rank**: Rank transformation replaces each feature abundance value by its
   rank. It is similar to relative rank transformation (rrank) which uses relative ranks.
...
change the data distribution [@Costea2014].
...
9. If the data has a phylogenetic tree, agglomerate the data to the family level and
perform the phILR transformation. Where was the transformed data stored?
Compare the feature names with original data. Why do the names differ?

Useful functions:

`utils::data()`, `miaViz::plotHistogram()`, `mia::transformAssay()`, `mia::assayNames()`,
`SummarizedExperiment::assay()`, `mia::agglomerateByRanks()`, `mia::altExp()`, `BiocGenerics::rownames()`
