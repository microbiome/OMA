...
# Function to create plots for the top 5 phyla
plot_top_phyla <- function(i) {
  # Convert to relative contributions for plotting
  drivers[[i]] <- drivers[[i]] / sum(drivers[[i]])
  
  # Sort by the relative contribution
  drivers_sorted <- drivers[order(drivers[[i]], decreasing = TRUE),]
  
  # Plot top 5 phyla
  ggplot(head(drivers, 5), aes(
...
# Generate both plots using lapply and combine them with wrap_plots
plots <- lapply(1:2, plot_top_phyla)
wrap_plots(plots)
```
...
we set the number of components to four instead of optimizing the number of
