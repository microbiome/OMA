...
we set the number of components `k` in four instead of optimizing the number of
...
```
```{r}
# Extract NMF feature loadings
loadings <- attr(res, "loadings")

# Add NMF loadings in the metadata
metadata(altExp(tse, "prevalent"))$NMF_loadings <- loadings
