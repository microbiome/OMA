...
plotHistogram(tse, row.var = "hclustKendall")
...
2. Follow the previous exercise and perform Principal Coordinate Analysis (PCoA)
...
4. Retrieve clusters and visualize the distribution of values with a bar plot.

5. Visualize PCoA results with points colored based on clusters.

Useful functions:

`transformAssay()`, `addMDS()`, `reducedDimNames()`, `reducedDim()`
`addCluster()`, `colData()`, `scater::plotReducedDim()`
