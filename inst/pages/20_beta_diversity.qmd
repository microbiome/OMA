...
The sequencing depth of a sample refers to the number of gene sequences read 
during the sequencing process. A variation in sequencing depth across the 
...
`GlobalPatterns` show up to a 40-fold difference in the number of gene sequences.
...
and if the rarefaction depth is set to the lowest sequence count found across all samples, no data will be omitted. 
Moreover, Patrick Schloss has demonstrated that rarefaction is "the only method that could control for variation in uneven sequencing effort when measuring commonly used alpha and beta diversity metrics" [@Schloss2023].

Before applying rarefaction, one can select the most variable features to reduce the variation caused by random subsampling. This approach allows for comparing the results between non-rarefied and rarefied distance calculations later on.

```{r, select highest variance features}
tse0 <- GlobalPatterns

# Let us see what happens when we operate with ntop highest variance features
ntop <- 20
tse0 <- tse0[head(rev(order(rowSds(assay(tse0, "counts")))), ntop), ]
```

Let us first convert the count assay to clr assay and calculate Aitchison distance without rarefaction:

```{r, Aitchison distance without rarefaction}
# Rarify data for comparisons

tse0 <- transformAssay(tse0,
                      assay.type = "counts",		      
                      method = "clr",
                      pseudocount = 1)
...
```{r, fig.cap = "MDS plot based on the Bray-Curtis distances using rarefaction with clr transformation on the GlobalPatterns dataset."}
...
tse0 <- runMDS(tse0, 
              ...
              sample=min(colSums(assay(tse0, "counts"))), # rarefaction depth 
              distfun=vegdist,  # Use vegdist beta diversity function
...
plotReducedDim(tse0, "MDS_aitchison_rarefied")

# Generate plots for non-rarefied and rarefied Bray-Curtis distances scaled by MDS
plots <- lapply(c("MDS_aitchison", "MDS_aitchison_rarefied"),
                plotReducedDim,
                object = tse0)
...
print(all.equal(current = cor(as.vector(reducedDim(tse0, "MDS_aitchison")),
                        as.vector(reducedDim(tse0, "MDS_aitchison_rarefied"))),
          target = 1,
          tolerance = 0.05))
```
...
The argument `sample` is set to the smallest sequence count across 
all samples. This ensures that no sample will be removed during the rarefaction 
process. 

The argument `iterations` is by default set to 100 but 10 iterations is often sufficient for beta diversity calculations.

To use transformations after the rarefaction of the samples and before the beta diversity calculation, `vegan::avgdist` has the argument `transf`.
...
for (k in 1:2) {
  ...
  pcx <- reducedDim(tse0, "MDS_aitchison")[,k]
  pcy <- reducedDim(tse0, "MDS_aitchison_rarefied")[,k]
...
would report relative stress, which varies within the unit interval and is better
