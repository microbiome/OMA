...
### Rarefaction to mitigate impact of uneven sequencing effort
...
For instance, the samples of the TreeSummarizedExperiment `GlobalPatterns` show 
up to a 40-fold difference in the number of gene sequences.
...
First, the user chooses the rarefaction depth and a number of iterations N. All the samples with sequence count below the rarefaction depth are removed and gene sequences are randomly drawn from other samples to reach the threshold. Then a beta diversity metric is calculated from those samples and the process is performed N times. Finally, beta diversity is estimated with the mean of all N beta diversity values calculated on subsampled data.

There has been a long-lasting controversy surrounding the use of rarefaction in
microbial ecology. The main concern was that rarefaction would omit data. 
However, if the subsampling process is repeated a sufficient number of times, 
and if the threshold is set to the lowest sequence count found across all samples, no data will be omitted. 
...
The function `avgdist` from the vegan package is a function that uses rarefaction to compute beta diversity.
...
tse <- runMDS(tse, 
              ...
              sample = min(colSums(assay(tse, "counts"))), # rarefaction depth
			        ...
			        iterations = 10 # Default at 100 (avgdist argument)
)
