...
The function `vegan::avgdist` uses rarefaction to compute beta diversity :

```{r, fig.cap = "MDS plot based on the Bray-Curtis distances using rarefaction on the GlobalPatterns dataset."}
...
tse <- runMDS(tse, 
              ...
              sample = min(colSums(assay(tse, "counts"))), # rarefaction depth
			        distfun = vegan::vegdist, # Use vegdist distance function
			        dmethod = "bray", # Bray-Curtis dissimilarity (vegdist argument)
			        iterations = 10 # 10 iterations should be enough for distance calculations
)

plotReducedDim(tse, "MDS")
```

`ntop` is a `runMDS` option. Here it is set to total number of features in `GlobalPatterns` dataset so that they are not filtered.

The argument `sample` is by default set to the smallest sequence count across 
all samples. This ensures that no sample will be removed during the rarefaction 
process.

The argument `iterations` is by default set to 100. 10 iterations should be 
...
# Run transformation after rarefactions before calculating the distances..
tse <- runMDS(tse, assay.type="counts",
                           ntop=nrow(GlobalPatterns), 
                           FUN = vegan::avgdist, # Custom distance that includes rarefaction
                           sample=min(colSums(assay(tse, "counts"))), # rarefaction depth 
			                     distfun=vegdist,  # Use vegdist distance function
                           dmethod = "euclidean", # euclidean dissimilarity
			                     iterations = 10, 
			                     transf=clr) 
...
The mia package has the rarefaction function `subsampleCounts` that randomly
subsamples one of the assays of a TreeSummarizedExperiment dataset. However, the 
iterations and dissimilarity calculations are not included and must be added 
manually.
...
comparison between the two designs in chapter
...
* dbRDA: `calculateRDA` and `runRDA`; our recommended default method
