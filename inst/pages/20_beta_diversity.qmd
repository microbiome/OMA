...
```{r, fig.cap = "Non-rarefied (left) and rarefied (right) MDS plots based on the Bray-Curtis distances on the GlobalPatterns dataset."}

tse <- runMDS(tse, 
        	assay.type="counts",
                ntop = nrow(GlobalPatterns),
        	FUN = vegan::avgdist,
                sample = min(colSums(assay(tse, "counts"))), # rarefaction depth
		distfun = vegan::vegdist, # Use vegdist beta diversity function
		dmethod = "bray", # Bray-Curtis beta diversity (vegdist argument)
		iterations = 10, # 10 iterations should be enough for beta diversity calculations
		name = "MDS_bray_rarefied",
)

# Generate plots for non-rarefied and rarefied Bray-Curtis distances scaled by MDS
plots <- lapply(c("MDS_bray", "MDS_bray_rarefied"),
                plotReducedDim,
                object = tse)
...
```
```{r rarefaction_2}
# Check correlation between non-rarefied and rarefied Bray-Curtis distances
all.equal(current = cor(as.vector(reducedDim(tse, "MDS_bray")),
                        as.vector(reducedDim(tse, "MDS_bray_rarefied"))),
          target = 1,
          tolerance = 0.05)
```
...
plotReducedDim(tse, "MDS_aitchison_rarefied")
```

```{r rarefaction_2}
# Check correlation between non-rarefied and rarefied Aichitson distances
all.equal(current = cor(as.vector(reducedDim(tse, "MDS_aitchison")),
                        as.vector(reducedDim(tse, "MDS_aitchison_rarefied"))),
          target = 1,
          tolerance = 0.05)
```

The mia package has the rarefaction function `rarefyAssay` that randomly
