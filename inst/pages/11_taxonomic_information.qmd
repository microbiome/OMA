...
Taxonomic information is a key part of analyzing microbiome data. Without
...
There are a number of methods to assign taxonomic information. Here you may find
...
```{r}
cat(paste(taxonomyRanks(tse), collapse = "\n"))
```

This can then be used to subset the `rowData` to columns needed.

```{r}
# Show only first lines and columns
rowData(tse)[, taxonomyRanks(tse)][1:5,1:4]
```
...
phylum <- !is.na(rowData(tse)$Phylum) &
    vapply(data.frame(apply(rowData(tse)[, taxonomyRanks(tse)[3:7]], 
                            1L, is.na)), all, logical(1))
...
tse <- transformAssay(tse, assay.type = "counts", 
                      method = "relabundance")
altExp(tse, "Family") <- mergeFeaturesByRank(tse, 
                                 rank = "Family",
...
assay(altExp(tse, "Species_byPrevalence"), 
      "relabundance")[88:92, 1:7]
```
...
# Cluster (with euclidean distance) on the features of the 
# z assay
tse <- addCluster(tse,
               assay.type = "z",
               clust.col = "hclustEuclidean",
	       MARGIN = "features",
               HclustParam(dist.fun = 
                             stats::dist, method = "ward.D2"))
...
# Cluster (with Kendall dissimilarity) on the features of the 
# z assay
tse <- addCluster(tse,
               assay.type = "z",
