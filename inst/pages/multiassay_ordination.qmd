...
The following chunk does not install and does not update `miniconda` if already
installed. Furthermore, it does not install the latest version of `mofapy2`.
If this setup causes issues, update `miniconda` and `mofapy2`.

```{r, message=FALSE, warning=FALSE}
#| label: mofapy2

reticulate::install_miniconda(force = FALSE, update = FALSE)
reticulate::use_miniconda(condaenv = reticulate::miniconda_path(), required = FALSE)
reticulate::py_install(
    packages = c("mofapy2"), pip = TRUE,
    pip_ignore_installed = TRUE, python_version=3.12
)
```
