...
tse.sub <- tse[ , tse$SampleType %in% 
                               c("Feces", "Skin", "Tongue")]

unique(tse.sub$SampleType)
```   
...
3.1. Find some samples and features to subset with.
...
3.2. Subset the TreeSE with your found samples and features.
...
tse_subset_by_feature <- tse[rowData(tse)$Phylum %in% features
                             ,
                             tse$SampleType %in% samples]
...
prevalentFeatures <- as.integer(getPrevalentFeatures(tse, detection = 1, 
                                          prevalence = 50/100))
# Subset the data to the prevalent features
tse.prevalent <- tse[rownames(rowData(tse)) %in% prevalentFeatures,]

# Alternatively subsetByPrevalentFeatures can be used to subset directly
tse.prevalent2 <- subsetByPrevalentFeatures(tse, detection = 1, 
                                           prevalence = 50/100)
identical(tse.prevalent,tse.prevalent2)
```  
...
length(assay(tse.prevalent, "counts"))
```  
...
prevalenceTse <- subsetByPrevalentFeatures(tse, detection = 0.1/100, 
                                           prevalence = 50/100)
...
plotRowData(prevalenceTse, "prevalence", 
            colour_by = "Phylum")
```   
...
2. Agglomerate data to all available taxonomy ranks by using `agglomerateByRanks()`
...
altExps(tse) <- agglomerateByRanks(tse)
...
5. Nevertheless, extract the p-values. Can the differences between samples be explained with variables from their metadata? 
```{r}
#| label: dbRDA-ex5
#| eval: FALSE
#| code-fold: true
...
2. Get the most prevalent features. Subset your TreeSe to those features and the first 5 taxa.
...
tse <- subsetByPrevalent(tse,prevalence=0.99999)[1:5,]
```
...
sechm(tse,features = most_prevalent , assayName="relabundance",        show_colnames=TRUE)
```
...
2. Agglomerate the data by prevalence to a rank of your choice, apply log10 transformation and assure unique rownames for when we plot the heatmap.
...
3. Calculate the correlations and their respective p-values between the log0 and count assays.
...
correlations <- getCrossAssociation(tse, 
...
```
Where : 
'reducedDim' is a function that extracts the reduced dimensionality representation of your data (e.g., from a PCA or 
PCoA object).
'plotReducedDim' is a function that can plot these reduced dimensions and has a 'colour_by argument' that accepts a vector of cluster assignments.
