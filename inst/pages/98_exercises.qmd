...
### 1: k-means Clustering on Reduced Dimensions and Visualization
...
```{r}
#| label: heatmap-visualization-ex3
#| eval: FALSE
#| code-fold: true
#| code-summary: "Show solution"

#| label: heatmap-visualization-ex1
#| eval: FALSE
#| code-fold: true
#| code-summary: "Show solution"
...
# Generate a sample SummarizedExperiment object
data(GlobalPatterns)
tse <- GlobalPatterns

# Perform MDS on the abundance data using scater::runMDS
# Specify the correct assay to be used
tse <- runMDS(tse, FUN = vegan::vegdist, method = "bray", assay.type = "counts")

# Perform k-means clustering (k = 3) using the bluster package
set.seed(123)
clustering_result <- clusterRows(reducedDim(tse, "MDS"), BLUSPARAM=KmeansParam(centers=3))
colData(tse)$cluster <- as.factor(clustering_result)
...
### 2: Cluster Analysis with Heatmap Visualization
...
```{r}
#| label: heatmap-visualization-ex2
#| eval: FALSE
#| code-fold: true
#| code-summary: "Show solution"
...
# Extract the abundance data matrix
abundance_data <- assay(se, "counts")

# Perform CLR transformation
clr_data <- t(apply(abundance_data, 1, function(x) {
  compositions::clr(x + 1)  # Adding 1 to avoid log(0)
}))

# Perform hierarchical clustering on the samples using Euclidean distance
distance_matrix <- dist(clr_data, method = "euclidean")
clustering <- hclust(distance_matrix, method = "average")

# Cut the dendrogram to form clusters (k = 3 for this example)
clusters <- cutree(clustering, k = 3)

# Add cluster information to colData
colData(se)$cluster <- as.factor(clusters)

# Visualize the heatmap with clusters using ComplexHeatmap
Heatmap(clr_data, 
        name = "CLR Counts",
        ...
        top_annotation = HeatmapAnnotation(Cluster = colData(se)$cluster, 
                                           col = list(Cluster = c("1" = "red", "2" = "blue", "3" = "green"))),
...
- Uses heatmaps to visualize high-dimensional data, such as abundance data.
- Annotations add context to the heatmap, linking clustering patterns to metadata like sample groups or experimental conditions.
...
# Ensure cluster information is correctly added to colData
# (Assuming the clusters have been calculated previously and added to colData)

# Create a sample metadata annotation (e.g., SampleType)
sample_metadata <- data.frame(SampleType = colData(se)$SampleType,
                              Cluster = colData(se)$cluster)
rownames(sample_metadata) <- colnames(assay(se, "counts"))

# Create a HeatmapAnnotation object
annotation <- HeatmapAnnotation(df = sample_metadata,
                                col = list(Cluster = c("1" = "red", "2" = "blue", "3" = "green")))

# Visualize the heatmap with annotations using ComplexHeatmap
Heatmap(assay(se, "counts"), 
        name = "Counts",
        row_dend_reorder = TRUE,
        column_dend_reorder = TRUE,
        show_row_names = FALSE,
