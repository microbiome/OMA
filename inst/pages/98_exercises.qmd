...
3. Import the scater library and run PCoA on on one of the created alternative experiments using Bray-Curtis dissimilarity.
...
rank <- "Genus"
# Calculate relative abundance
altExp(tse, rank) <- transformAssay(altExp(tse, rank), MARGIN = "samples", method="relabundance")
...
4. Plot the first two coordinates using the `reducedDim` function and plot the coordinates.
...
5. Plot similar heatmap but adjust color scale to ensure that 0 is in the middle
