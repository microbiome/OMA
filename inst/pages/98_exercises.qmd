...
In this exercise, we'll go over the basics of subsetting a TSE object.
Keep in mind that it's good practice to always keep the original data intact.
Therefore, we suggest creating a new tse object whenever you subset.
...
# First, merge the features by rank
...
prevalentFeatures <- getPrevalentFeatures(altExp(tse,"Phylum"), detection = 0, 
                                          prevalence = 50/100)
...
prevalenceTse <- subsetByPrevalentFeatures(altExp(tse,"Phylum"), detection = 0, 
                                          prevalence = 50/100)
```  
...
altExp(tse,"Phylum2") <- mergeFeaturesByRank(tse, "Phylum") 

prevalenceTse <- subsetByPrevalentFeatures(altExp(tse,"Phylum2"), detection = 0, 
                                           prevalence = 50/100)
