...
2. Get the most prevalent features. Subset your TreeSe to those features and the first twenty taxa.
...
most_prevalent <-  names(head(getPrevalence(tse,sort=TRUE)))

tse <- tse[most_prevalent, 1:20]
```
...
5. Visualize the relative abundances with a heatmap using the sechm library
