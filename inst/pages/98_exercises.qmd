...
data(GlobalPatterns)
se <- SummarizedExperiment(assays = list(counts = assay(GlobalPatterns)),
                           colData = colData(GlobalPatterns),
                           rowData = rowData(GlobalPatterns))

# Perform PCoA on the abundance data
distance_matrix <- vegdist(t(assay(se)), method = "bray")
pcoa_results <- cmdscale(distance_matrix, k = 2)
pcoa_data <- data.frame(pcoa_results)
colnames(pcoa_data) <- c("PCoA1", "PCoA2")

# Perform k-means clustering (k = 3)
set.seed(123)
kmeans_result <- kmeans(pcoa_data, centers = 3)
pcoa_data$cluster <- as.factor(kmeans_result$cluster)

# Visualize PCoA with clusters
ggplot(pcoa_data, aes(x = PCoA1, y = PCoA2, color = cluster)) +
  geom_point(size = 3) +
  labs(title = "PCoA with k-means Clustering", x = "PCoA Axis 1", y = "PCoA Axis 2") +
  theme_minimal() +
...
distance_matrix <- dist(t(abundance_data), method = "euclidean")
...
# Visualize the heatmap with clusters
heatmap_data <- assay(se, "counts")
pheatmap(heatmap_data, 
         cluster_rows = clustering, 
         cluster_cols = clustering, 
         annotation_col = data.frame(Cluster = colData(se)$cluster),
...
rownames(sample_metadata) <- colnames(abundance_data)

# Visualize the heatmap with annotations
pheatmap(abundance_data, 
