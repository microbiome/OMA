...
**Extra** Cool exercise : Identifying patterns and clusters.
...
tse <- addCluster(tse, clustering_result, colLabel = "cluster")
...
# CLR transformation of abundance data matrix and add it to the tse object
clr_data <- transformAssay(tse, method = "clr", assay.type = "counts", pseudocount = TRUE)
assay(tse, "clr") <- as.matrix(clr_data)

# Hierarchical clustering on samples
clustering <- hclust(dist(assay(tse, "clr"), method = "euclidean"), method = "average")
clusters <- cutree(clustering, k = 3)
tse <- addCluster(tse, clusters, colLabel = "cluster")
...
sechm(
  ...
  row_labels = FALSE,
  column_labels = FALSE,
  row_cluster = clustering,  # Use clustering object directly
  column_cluster = clustering,  
  row_annotation = as.data.frame(colData(tse)),
...
sechm_plot <- sechm(assay(tse, "counts"),
                    annotation_col = as.data.frame(colData(tse)),
