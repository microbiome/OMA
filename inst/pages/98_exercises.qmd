...
row_data <- DataFrame(GeneID = 1:20) # Row metadata
phylo_tree <- rtree(n=20) # Generating a random phylogenetic tree for demonstration

# Extract sample metadata for annotation directly from the tse object
sample_metadata <- colData(tse)   

# Create TreeSummarizedExperiment object
tse <- TreeSummarizedExperiment(assays = list(counts = example_data),
                                       colData = col_data,
                                       rowData = row_data,
                                       rowTree = phylo_tree)

# Generate heatmap with clustering and sample annotations
heatmap <- draw(tse, data = "counts", metaData = "Group", showRowDendro = TRUE, 
               showColDendro = TRUE, cluster_rows = TRUE, cluster_cols = TRUE,
