...
# CLR transformation of abundance data matrix
clr_data <- as.matrix(transformAssay(tse, method = "clr", assay.type = "counts", pseudocount = TRUE))

# Hierarchical clustering on samples
clustering <- hclust(dist(clr_data, method = "euclidean"), method = "average")
clusters <- cutree(clustering, k = 3)
colData(tse)$cluster <- as.factor(clusters)

# Create annotation for clusters
annotation_df <- data.frame(Cluster = colData(tse)$cluster)
annotation_colors <- list(Cluster = c("1" = "red", "2" = "blue", "3" = "green"))

# Visualize the heatmap with sechm
sechm(
  mat = clr_data,
  row_labels = FALSE,
  column_labels = FALSE,
  row_cluster = clustering,
  column_cluster = clustering,
  row_annotation = annotation_df,
  row_annotation_colors = annotation_colors,
...
# Create a sample metadata annotation (e.g., SampleType and Cluster)
sample_metadata <- data.frame(SampleType = colData(tse)$SampleType,
                              Cluster = colData(tse)$cluster)
rownames(sample_metadata) <- colnames(assay(tse, "counts"))

# Visualize the heatmap with annotations using sechm
sechm_plot <- sechm(assay(tse, "counts"),
