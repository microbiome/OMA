...
*bluster* is a Bioconductor package providing tools for clustering data in the `SummarizedExperiment` container. It offers multiple algorithms such 
...
cluster enterotype data. The `addCluster` function allows to choose a clustering algorithm and offers multiple parameters to shape the result. 
...
A parameter, `MARGIN`, defines whether to cluster features or samples .
...
# Plot the clusters
plotReducedDim(altExp(tse, "hclust"), "MDS", colour.by = "clusters")
```
