...
threshold (`as.relative = FALSE`) at read count 1 (`detection = 1`) is
accessed.
...
Library sizes other variables from `colData` can be visualized by using a specified function called `plotColData`.
...
colData(tse) <- as.data.frame(colData(tse)) %>%
        	 ...
        	 mutate(X.SampleID = factor(X.SampleID, 
        	                            levels=X.SampleID)) %>%
		 DataFrame
plotColData(tse,"sum","X.SampleID", colour.by = "SampleType") + 
...
plotColData(tse,"sum","SampleType", colour.by = "SampleType") + 
    theme(axis.text.x = element_text(angle = 45, hjust=1))
```
