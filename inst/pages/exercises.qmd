...
1. Load the necessary libraries and import the dataset into R; here we are
using a microbiome dataset.
...
2. Calculate Alpha Diversity (Shannon Index), i.e. the dependent variable.

```{r}
#| label: extra-regressionchart-step2
#| code-fold: true
#| code-summary: "Show solution"
#| eval: true
...
Now let's define the covariates i.e. the independent variables.

3. First, fit linear model using a discrete variable, say "SampleType", where 
Shannon diversity is the response variable, and SampleType is the predictor. 

"SampleType" represents the type or source of the microbiome sample. It 
categorizes each sample based on the environment or biological material from
which it was collected.

```{r}
#| label: extra-regressionchart-step3
#| code-fold: true
#| code-summary: "Show solution"
#| eval: true
...
```{r}
#| label: extra-regressionchart-step4
#| code-fold: true
#| code-summary: "Show solution"
#| eval: true
...
b. The Normal Q-Q plot assesses whether the residuals are normally distributed. In this
plot, residuals should align closely with the diagonal line. Deviations from this line,
especially at the tails, indicate that the residuals are not normally distributed, 
which could violate the assumption of normality.
c. The Scale-Location (or Spread-Location) plot also helps check the homoscedasticity 
assumption. It plots the square root of standardized residuals against the fitted values.
d. The Residuals vs Leverage plot helps identify influential data points (outliers) that 
might disproportionately affect the model. Points outside the dashed lines (Cook’s 
...
5. Further, fit linear model using a continuous variable, say "coverage", where
Shannon diversity is the response variable, and coverage is the predictor. 
...
```{r}
#| label: extra-regressionchart-step5
#| code-fold: true
#| code-summary: "Show solution"
#| eval: true
...
6. Visualize this model with a scatterplot. In R, you can also use 
[`ggplot2`](https://ggplot2.tidyverse.org/) for
plotting and `lm()` function for linear modeling.
```{r}
#| label: extra-regressionchart-step6
#| code-fold: true
#| code-summary: "Show solution"
#| eval: true
...
where the slope of the regression line indicates the rate of change in Shannon 
diversity per unit change in coverage unlike our previous example in which the predictor
is categorical (like SampleType) and coefficients represent differences
between group means.

8. Summarize results with `gtsummary`. The 
[`gtsummary`](https://www.danieldsjoberg.com/gtsummary/) package is used to generate
a summary table of the regression model, which is easy to interpret and
present in reports or publications.
```{r}
#| label: extra-regressionchart-step8
#| code-fold: true
#| code-summary: "Show solution"
#| eval: true
...
9. Visualize coefficients with `jtools`. The 
[`jtools`](https://cran.r-project.org/web/packages/jtools/index.html) package
provides a visual representation of the model’s coefficients, including their 
confidence intervals, allowing for a clear understanding of the effect sizes.
```{r}
#| label: extra-regressionchart-step9
#| code-fold: true
#| code-summary: "Show solution"
#| eval: true
...
and [`scater`](https://bioconductor.org/packages/release/bioc/html/scater.html)
can be used.
