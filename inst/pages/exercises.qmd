...
# Create example data
set.seed(123)
example_data <- matrix(rnorm(200), nrow = 20, ncol = 10)
...
mds_data <- reducedDim(tse, "MDS")

# Perform k-means clustering (k = 3) using the base R kmeans function
set.seed(123)
kmeans_result <- kmeans(mds_data, centers = 3)

# Create KmeansParam object
kmeans_param <- KmeansParam(centers = 3)

# Add cluster labels to the SummarizedExperiment object using mia::addCluster()
tse <- addCluster(tse, cluster = kmeans_result$cluster, BLUSPARAM = kmeans_param)
...
# Perform hierarchical clustering on the MDS data (extracted in last exercise)
dist_matrix <- dist(mds_data)  # Compute distance matrix on MDS dimensions
hclust_result <- hclust(dist_matrix, method = "complete")

# Cut the dendrogram into 3 clusters
cluster_labels <- cutree(hclust_result, k = 3)

# Add cluster labels to the SummarizedExperiment object
colData(tse)$cluster <- factor(cluster_labels)

# Visualize MDS with clusters using scater::plotReducedDim
plotReducedDim(tse, dimred = "MDS", colour_by = "cluster") +
  labs(title = "MDS with Hierarchical Clustering") +
  theme_minimal()

# Optional: Visualize the hierarchical clustering with a heatmap
# Convert MDS data to a matrix for heatmap visualization
mds_matrix <- t(as.matrix(mds_data))  # Transpose the matrix to match the columns

# Create a heatmap with cluster annotations
heatmap <- Heatmap(mds_matrix,
                   name = "MDS",
                   show_row_dend = TRUE,
...
Customize the heatmap with a specific color palette to enhance visualization using ComplexHeatmap.
...
custom_colors <- colorRamp2(c(min(example_data), 0, max(example_data)), c("blue", "white", "red"))
