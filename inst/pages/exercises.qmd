...
5. Create a basic heatmap using the `pheatmap` package.
...
# Create heatmap using pheatmap on mia's enterotype dataset
pheatmap(assay(enterotype, "counts"))
```
...
1. Create an interactive heatmap. 
...
heatmaply(assay(agglomerateByRank(GlobalPatterns, rank = "Phylum")),
          xlab = "Samples", ylab = "Phyla")
```
Notice we aggregate the "GlobalPatterns" dataset
to a higher taxonomic level to make the data more manageable.
...
tse <- addCluster(enterotype, colDataName = "Cluster", BLUSPARAM = hclust_param)
...
4. Customize the heatmap with a specific color palette to enhance visualization.
...
custom_colors <- colorRamp2(c(min(assay(esophagus)), mean(assay(esophagus)), 
                            max(assay(esophagus))), c("blue", "white", "red"))

Heatmap(assay(esophagus),  
        col = custom_colors,
        cluster_rows = TRUE, 
        cluster_columns = TRUE)
```
Sometimes, e.g. with `sechm`, you might have to do log transformation to reduce the skewness and 
bring the higher values closer to the rest of the data. This will help in highlighting
differences across the entire range of data.
