...
5. Create a basic heatmap of the data using the `pheatmap` package.
...
# Load example dataset from mia package
data(enterotype)

# Extract the data matrix for heatmap
example_data <- assay(enterotype, "counts")

# Create basic heatmap using pheatmap
...
# Load the GlobalPatterns dataset from the mia package
data("GlobalPatterns", package = "mia")

# Here we extract the first 20 taxa and the first 10 samples as an example
taxa_data <- assay(GlobalPatterns)[1:20, 1:10]
taxa_names <- rownames(taxa_data)
sample_names <- colnames(taxa_data)

# Create the interactive heatmap using heatmaply
heatmaply(taxa_data,
          xlab = "Samples",
          ylab = "Taxa",
          main = "Interactive Heatmap of
...
tse <- runMDS(tse, FUN = vegan::vegdist, method = "bray", assay.type = "counts")

# Perform k-means clustering using mia's addCluster function
kmeans_param <- KmeansParam(centers = 3)
tse <- addCluster(tse, colDataName = "kmeans_clusters", BLUSPARAM = kmeans_param)

# Visualize MDS with clusters
plotReducedDim(tse, dimred = "MDS") +
...
# Here we extract the first 20 taxa and the first 20 samples as an example
sample_data <- assay(GlobalPatterns)[1:20, 1:20]

# Create a SummarizedExperiment object
se <- SummarizedExperiment(assays = list(counts = sample_data))

# Perform hierarchical clustering using bluster package & addCluster from mia package
hclust_param <- HclustParam(method = "complete", cut.params = list(k = 3))
se <- addCluster(se, 
                 colDataName = "Cluster", 
                 BLUSPARAM = hclust_param)
...
4. Heatmap with Custom Color Palette:
Customize the heatmap with a specific color palette to enhance visualization 
...
# Here we extract the first 30 taxa and the first 20 samples as an example
example_data <- assay(GlobalPatterns)[1:30, 1:20]

# Define custom color palette
custom_colors <- colorRamp2(c(min(example_data), 0, max(example_data)), c("yellow", "green", "red"))

# Create heatmap with custom color palette
Heatmap(example_data, col = custom_colors)
```
