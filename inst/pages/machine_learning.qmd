...
transformations &mdash;such as relative abundance or Hellinger&mdash;
often outperform compositional transformations like CLR when building
Random Forests [@yerke2024_transformationsML], one of the algorithms 
discussed in the next section. The authors note that the impact of the 
chosen transformations decreases as the signal-to-noise ratio 
...
model performance. A similar relationship has been observed in 
regression tasks. Moreover, in addition to improving model performance, 
proportion-based transformations are more intuitive and therefore easier
to interpret.
...
zero, which indicates a small contribution of each taxa in allowing the
...
patients (see Figure 2A). This taxa was also reported as having 
