...
model's performance. While this approach is not as reliable as having
a separate test set, it can still give valuable insights into how well
the model might perform on new data. 
...
research article where the dataset was first described [@qin2012_t2d].
...
* **Collapse highly correlated features:** It's common for two or 
more taxa to show high correlation in their abundances within a 
community. Thus, removing or collapsing correlated taxa allows the
...
and taxa present in just a few samples of each group hardly provide 
useful biological information for their classification. 
...
The code below shows how to join the abundance of each microbial taxa
...
In the code below we join the BMI of each participant instead. We
can do so by just changing the outcome variable of interest. Again,
the resulting preprocessed dataset will be then used to train and test
our model.
...
The code above should be deemed as a reference. Preprocessing 
strategies impact the performance of ML models, and thus the steps
used should always be tailored to satisfy specific research goals. 
Other preprocessing steps can be implemented using it the
`preprocess_data` function, as described
...
Possible reasons for that are that they can be used for classification 
...
of the features (e.g., microbial taxa). Each tree finds the best
...
BMI) based on the values of the features (e.g., microbial taxa). Each 
...
in the data that other regression algorithms might not capture.
...
Although XGBoost can be used for classification and regression
tasks, only the first is covered in this chapter. When applied to 
classification, each tree predicts the class of an observation (e.g., 
healthy or T2D) based on feature values (e.g., microbial taxa). However,
instead of 'voting' like in Random Forest, XGBoost improves the 
performance by assigning higher weights (or penalties) to missclassified 
observations.
...
Under the hood, the function `run_ml` generates a 80/20 split of 
the data to train and test a model of interest. That means 
...
Thus, in this section we will discuss different metrics used to 
assess the performance of the model. However, notice that we only 
use the function `run_ml` one time per model (i.e., we performed only
one 80/20 split). Thus, the metrics we will discuss represent the 
performance of the model in one of multiple splitting scenarios. Often, 
researchers are interested in generating multiple splits, calculate 
the performance for each model, and then look at the variability across 
iterations. This gives a more accurate assessment of the model's 
performance. 

That approach is discussed in the 
[documentation](https://www.schlosslab.org/mikropml/articles/parallel.html) 
of `mikropml`. Therefore, we will focus in interpreting the results
of one split. The goals below are to discuss the different performance 
metrics, to compare metrics from two models used in the classification 
of the same observations, and to contrasts the results of these models
with previous analyses in the published literature.
...
A common metric to quickly assess model performance is the area under 
the receiver operator characteristic curve 'AUC'. Notice that
the result tables include two types of AUC metrics: `cv_metric_AUC`,
which represents AUC for the 80% of data used in training, and `AUC`, 
which is the AUC for the 20% used in testing. 

Researchers are often interested in scenarios where `AUC` is as good as 
or better than `cv_metric_AUC`. The opposite suggests that the model may 
be overfitted, meaning that the model fails to generalize to new data.
Notice, however, that small drops of model's performance in the test 
compared to the train data are acceptable and shouldn't be a concern.
...
to train different models and compare them to each other.

Now let's interpret our results at the light of previous research.
In the largest yet published multi-cohort analysis of gut microbiome 
associations to T2D [@mei2024_t2d], authors report similar AUC values 
...
working with in their analysis. Notice their reported AUC of 0.74, 
which is very close to ours. Together, this further validates the 
potential of the gut microbiome to discriminate between
healthy individuals and T2D patients.

Although the tasks we addressed required binomial classification (i.e., 
classify either as patients or controls), some research questions
...
allowing the assessment of model performance in multiclass problems.
...
easily achieve 95% accuracy by just predicting all samples as healthy, 
despite being useless for the classification of T2D patients. 
...
for the 20% used in testing. Again, researchers are often interested 
in scenarios where `RMSE` is as good as or better than 
`cv_metric_RMSE`. Notice, however, the small drop of model's performance
in the test (`{r} round(rf_regression[["performance"]][["RMSE"]], 3)`)
compared to the train 
(`{r} round(rf_regression[["performance"]][["cv_metric_RMSE"]], 3)`)
data. As discussed above, this shouldn't be a concern given that the 
drop is small.
...
model's performance in classification tasks. As it name suggests,
...
of taxa and alpha diversity metrics, as we did). However, it is 
...
Or in terms of the PRC plot, the perfect model would consistently 
...
we may conclude that gut microbiome composition is not very 
informative of BMI.
...
understand how different features (e.g., microbial taxa) affect model's 
performance is by randomly permuting the values of that feature across 
samples and then evaluating the changes in a performance metric.
...
ggplot(feat_imp) +
    ...
    labs(x = "Performance reduction", y = "Feature") +
    theme_classic()
```
...
However, note that the differences in model performance are very close 
to zero, which indicates a small contribution of each taxa in allowing the
model to distinguish between groups. 

Note that these results are coherent with what authors of the 
...
patients (see Figure 2A). This same taxa was also reported as having 
the largest differences in the mean abundances between groups
[@mei2024_t2d] (see Figures 2A and 2B). 

::: {.callout-important}
## Important: A call to avoid oversimplifications

These plots are often interpreted in the literature with a reductionist 
mindset. Thus, researchers often conclude that a single feature (i.e., 
bacterial taxa) is responsible of a clinical output of interest. However,
more often than not, microbes interact with one another, and thus the 
effect of a single taxa doesn't always inform about ecosystem-level
properties of the total microbiome. It is important to bear that
in mind when interpreting the results of our models.
:::