...
is a list of models well supported by mikropml.
...
microbial taxa). However, instead of 'voting' like in Random Forest, 
...
Therefore, the goals of this section is to discuss different metrics
...
the performance for the models trained on each split, and then 
...
It was discussed above that ROC curves (and AUCs) can be extender to 
...
the R package `pROC` supports the visualization of multiclass ROC curves
:::
...
from the data. However, if such patterns don't exist it would be 
impossible for the model to correclty predict an outcome. This is
most likely what happened in this regression tasks but it can 
certainly happen in classification tasks too.
