...
'transformSamples' includes following transformation methods:
...
-   presence/absence (pa)
-   rank (rank)
-   relative (relabundance)

'transformFeatures' includes following transformation methods:
...
Following prameters are used as an input: 

-   'x' is a SummarizedExperiment object

-   'abund_values' specifies the abundance table where transformation is applied.

-   'method' specifies the transformation method that is used. By default, 
transformation is applied to 'counts' table.

-   'pseudocount' specifies the value that is added to data. It must be added when
logarithmic transformation is done (clr, hellinger, log10), and the data 
contains zeros.

-   'name' specifies the name of transformed abundance table. It is optional, but it
is preferred. By default, name is same as the method that is used.

```{r}
se <- transformSamples(x = se, abund_values = "counts", method = "log10", 
                       pseudocount = 1, name = "log10_transformation")

head(assay(se, "log10_transformation"))
```
...
```{r}
# list of abundance tables hat assays slot contains
assays(se)
```
