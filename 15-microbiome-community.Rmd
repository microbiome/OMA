...
Here, clr (center log ratio) transformed abundances are plotted at Phylum level. 

```{r heatmap}
library(ggplot2)
tse_phylum <- agglomerateByRank(tse, rank = "Phylum")
tse_phylum <- transformSamples(tse_phylum, method = "clr", pseudocount = 1)
df <- meltAssay(tse_phylum, assay_name = "clr")

ggplot(df, aes(x = SampleID, y = FeatureID, fill = clr)) +
    geom_tile() +
